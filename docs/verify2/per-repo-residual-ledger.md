...
| vapor-api-template | swift | 21 | 2.13% (2026-05-19, chain-fix #491 — basename-only source path support) | chain-fix #491 (looksLikeSourceFilePath accepts basename-only source paths) | residual = 0 bug-extractor + 2 bug-resolver (`App` SwiftPM target-dependency name collides with local SCOPE.Component created by import-extractor) | at-bar | swift extractor: stop emitting SCOPE.Component per import (the same name leaks into the local entity table and causes bug-resolver ambiguity) — file swift-import-extractor issue |
