...
| click | python | 138 | 6.45% (2026-05-19, post-#525 confirmed flat; was 6.86%) | python-django-w4 + #525 | resolver tier-1 real-vs-SCOPE preference confirmed flat on click (no Component/SCOPE.Component same-file collisions on EXTENDS). Shared cumulative wins | at-bar | next python wave for ship-gate gap |
| django-realworld | python | — | 4.72% (2026-05-19, post-wave-6; was 4.91% post-#526, 7.83% python-django-w4) | wave-6 spillover delta: -0.19pp (datetime / `getlist` / `order_by` allowlist additions classify Django ORM bare-names already present in django-realworld). Residual: DRF inherited methods `paginate_queryset`/`get_paginated_response` from `ListAPIView` (external-known classification), Django URLConf binder (#527), generic verbs (#529) | at-bar | #527 URLConf, DRF base-class external-known allowlist — path to ~1.8% |
| pandas | python | 197 | 10.11% (2026-05-19, post-#525 confirmed flat; was 13.85%) | python-django-w4 + #525 | resolver tier-1 real-vs-SCOPE preference confirmed flat on pandas. Wave-4 numpy/pandas bare-names (asarray=196/astype=195/is_list_like=145/find_stack_level=109/arange=80/zeros=74/Timestamp/Timedelta/AbstractMethodError/etc.) — top stubs accumulated 80-200 hits each pre-fix | structural | further pandas wave needed: `view`/`func`/`cls`/`equals`/`reindex`/`op` collision-prone, dotted `type._simple_new` receiver-binding |
| client-fixture-a | python (Django backend, user-provided) | 4202 | 9.80% (2026-05-19, post-wave-6 PyMongo+datetime; was 12.00% post-#526, 12.56% post-#507+#521, 13.70% pre-fixes) | wave-6 python pymongo+datetime+django-extras bare-names — measured delta: bug-extractor 3300 → 2619 (-681), bug_rate 11.997% → 9.799% (-2.20pp). Allowlist gained 47 PyMongo Collection methods (`get_collection`, `insert_one`, `update_one`, `delete_one`, `find_one`, `count_documents`, `bulk_write`, `create_index`, ...), bson `Decimal128`, datetime/dateutil (`strptime`, `fromisoformat`, `utcnow`, `now`, `today`, `timedelta`, `relativedelta`), random (`randint`, `randrange`), uuid (`uuid4`, `uuid1`), requests `raise_for_status`, Django QueryDict `getlist` + QuerySet `order_by`, DB-API `fetchall`/`fetchone`/`fetchmany`. Top residual buckets after wave-6: generic verbs (`append`=163, `first`=133, `items`=75, `replace`=47, `find`=43) explicitly excluded by #94 safer-bias rule, plus project-internal calls (`SUCCESS`, `setMessageParams`, `permission`=31, `get_text`=15, `send_message`=13, `get_device`) that require receiver-variable-type inference (#494 / #543). | at-bar (≤10%) | Bulk of remaining residual is project-internal receiver-stripped method calls — requires receiver-variable-type tracking (#494) or v1.0 residual-repair pass (#543); generic verbs intentionally not allowlisted per safer-bias rule |
