...
| flask-realworld | python | 43 | 7.60% (2026-05-19, python-django-w4; was 13.81%) | python-django-w4 | django/DRF/flask/SQLA framework base-class allowlist for EXTENDS structural-refs + Wave-4 pythonBareNames (is_valid/set_password/check_password/post_json/get_by_id/...) + knownExternalPackages (flask_sqlalchemy/flask_jwt_extended/pymongo/psycopg2/...) + super.X dynamic regex. Residual: SQLA `Query.first()` (ruby/swift collision blocks lang gate), generic verbs `cls`/`pop`/`items`/`remove`/`append` (too collision-prone), `User.save`/`UserProfile.save` dotted-receiver class member access (extractor receiver-binding) | at-bar | dotted-receiver class member binding + ruby-collision-free `first` route (chain-fix) |
| click | python | 138 | 6.45% (2026-05-19, python-django-w4; was 6.86%) | python-django-w4 | shared pythonBareNames + knownExternalPackages additions reach click via cumulative wins | at-bar | next python wave for ship-gate gap |
| django-realworld | python | 48 | 7.83% (2026-05-19, python-django-w4; was 13.11%) | python-django-w4 | django/DRF framework base-class allowlist (`APIView`/`RetrieveAPIView`/`AppConfig`/`JSONRenderer`/`AbstractBaseUser`/`PermissionsMixin`/`BaseUserManager`/`SQLAlchemyModelFactory`) + Wave-4 pythonBareNames (is_valid/is_authenticated/set_password/follow/unfollow/favorite/has_favorited/exception_handler/...) + super.X dynamic regex + bare-kind-prefixed lang="" allowlist fallback. Residual: `*.serializer_class` / `*.get_queryset` dotted ViewSet attribute access (extractor receiver-binding), `Route:conduit.apps.X.urls` cross-language URLConf includes, generic verbs (`pop`/`items`/`now`/`choice`/`timedelta`) | at-bar | DRF ViewSet attribute receiver-binding (chain-fix) |
| pandas | python | 197 | 10.11% (2026-05-19, python-django-w4; was 13.85%) | python-django-w4 | wave-4 numpy/pandas bare-names (asarray=196/astype=195/is_list_like=145/find_stack_level=109/arange=80/zeros=74/Timestamp/Timedelta/AbstractMethodError/etc.) — top stubs accumulated 80-200 hits each pre-fix | structural | further pandas wave needed: `view`/`func`/`cls`/`equals`/`reindex`/`op` collision-prone, dotted `type._simple_new` receiver-binding |
