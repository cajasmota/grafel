...
| click | python | — | 5.47% (2026-05-19, flask-realworld wave spillover; was 5.96% pre-wave, 6.05% post-wave-9 builtin-type methods, 6.12% post-wave-8, 6.45% post-#525, 6.86% pre-fix) | flask-realworld wave spillover -0.49pp (Click DSL bare-names echo/secho/prompt/confirm/clear/argument/option/pass_context/pass_obj/make_pass_decorator + Python builtin exceptions in pythonExternalBaseTypes + isPythonLocalDottedReexport classifies click submodule re-exports as Dynamic + scope:endpoint ref index resolves click CLI handlers) | at-bar | next python wave for ship-gate gap |
