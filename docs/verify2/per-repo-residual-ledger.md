...
| requests | python | 111 | 1.51% (2026-05-19, python-django-w5; carried from w4) | python-django-w5 | clean | at-bar | within striking distance of 1% — push for ship-gate |
| flask-realworld | python | 43 | 5.85% (2026-05-19, python-django-w5; was 7.60% w4) | python-django-w5 | EXTENDS/IMPLEMENTS kind-disambiguator landed via #556 (nameKindsReal/byLocationKindReal tier-1 real-vs-SCOPE preference); this PR adds DEPENDS_ON/IMPORTS hint-family + LookupStatusHint kind-mismatch fallback + cross-file Format A fallback (lookupGlobalByFamily), dotted ClassName.save/.delete/.to_json class-attribute classifier (pythonClassAttrs), Python builtin exceptions in pythonExternalBaseTypes, Route:* URLConf→Dynamic, `first` claimed by pythonBareNames. Residual: `cls`/`walk`/`remove`/`post`/`pop`/`offset`/`limit`/`exit`/`append` generic verbs (collision-prone), `now`/`utcnow` bare instance helpers | at-bar | one more wave for ≤1% (generic-verb allowlist with receiver-type gate) |
| click | python | 138 | 6.34% (2026-05-19, python-django-w5; was 6.45% w4) | python-django-w5 | cumulative wave-5 wins (`first`+Python builtin exceptions in pythonExternalBaseTypes) | at-bar | next python wave for ship-gate gap |
| django-realworld | python | 48 | 2.84% (2026-05-19, python-django-w5; was 7.83% w4) | python-django-w5 | #556 disambiguator + this PR: cross-file Format A fallback (lookupGlobalByFamily), kind-mismatch hint fallback for DEPENDS_ON/IMPORTS, DRF dotted class-attribute / inherited-method classifier (pythonClassAttrs: serializer_class/get_queryset/paginate_queryset/etc. → Dynamic when receiver is a Component-family entity), Django URLConf Route:* → Dynamic, Python builtin exception classes in pythonExternalBaseTypes. Residual: 2 `remove`, 1 each of `pop`/`now`/`items`/`timedelta`/`authenticate`/`UserManager.normalize_email`/`UserManager.model` (UserManager is a Service entity, currently outside componentKindFamily). | at-ship-gate | extend nameExistsInComponentFamily to include Service kind (one-line chain-fix) |
| pandas | python | 197 | 9.89% (2026-05-19, python-django-w5; was 10.11% w4) | python-django-w5 | wave-4 numpy/pandas bare-names carry through; wave-5 kind-disambiguator + Python builtin exceptions help slightly | structural | future pandas wave needed for dotted-receiver class-attribute access |
| client-fixture-a | python (Django backend, user-provided) | 4202 | 12.45% (2026-05-19, post-#526; was 12.56% post-#507+#521, 13.70% pre-fixes) | #526 (python class-attr field entities) — measured delta: bug-extractor 3335 → 3300 (-35). #507 (psycopg2 dataaccess) + #521 (python wave-4 cumulative) landed prior. Top residual buckets unchanged: queryset method verbs (`order_by`, `update_many`, `permission`), `get_object_or_404` (Django shortcut module — needs external-known), `get_device` (project-internal cross-file resolution). Chain-fixes #531 (external-sql disposition bucket), #532 (db:&lt;table&gt; entity synthesis) | structural | Bulk of residual is python bare-other CALLS — addressable via Mongo/Celery follow-up + Django-shortcuts allowlist |
