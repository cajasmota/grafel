...
| vapor-api-template | swift | 21 | 2.13% (2026-05-19, #491 + #492) | chain-fix #491 (looksLikeSourceFilePath basename-only) + #492 (this PR — swift import-extractor namespaces SCOPE.Component carrier as `<file>::import::<module>` and tags Subtype="module", eliminating the `App` collision) | bug-resolver count: 2 → 0 (collision resolved). bug-extractor count: 0 → 2 (the `App` SwiftPM target-dependency IMPORTS edges no longer false-resolve to the carrier shim; they're now honestly counted as missing — they need the Package.swift target-extractor to bind to a real entity). Total bug count unchanged at 2 / bug-rate unchanged at 2.13%, but the disposition is now correct. | at-bar | ship a SwiftPM target-extractor for `Package.swift` so the `App` target declares a SCOPE.Component the import binds to → drives bug-rate to 0%. |
| sample-food-truck | swift | — | — | — | — | unmeasured | clone + index |
| vapor | swift | ~250 | 14.27% (2026-05-19, post-#492) | #492 (this PR — same swift import-carrier namespacing fix) | bug-resolver 178 → 88 (-90); bug-extractor 526 → 627 (+101); resolved 3122 → 3108 (-14). Net +11 bugs / +0.22pp on the headline rate (14.05% → 14.27%). The previously-resolved edges were spuriously binding bare-name CALLS / REFERENCES to the import-carrier shim (e.g. `Foundation.foo()` matching the local `Foundation` carrier instead of the real external symbol); removing that shim moves them to bug-extractor where they belong. The dispositions are now more honest even though the absolute rate ticks up. | regression-by-honesty | (a) extend the Swift external-known allowlist to absorb the freed-up bare-name CALLS targets that lost their shim resolution; (b) namespace-imported-name resolution would recover those 14 edges as honest external-known. |
