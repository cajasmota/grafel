...
| flask-realworld | python | 43 | 5.85% (2026-05-19, python-django-w5; was 7.60% w4) | python-django-w5 | EXTENDS/IMPLEMENTS kind-disambiguator landed via #556 (nameKindsReal/byLocationKindReal tier-1 real-vs-SCOPE preference); #553 adds DEPENDS_ON/IMPORTS hint-family + LookupStatusHint kind-mismatch fallback + cross-file Format A fallback (lookupGlobalByFamily), dotted ClassName.save/.delete/.to_json class-attribute classifier (pythonClassAttrs), Python builtin exceptions in pythonExternalBaseTypes, Route:* URLConf→Dynamic, `first` claimed by pythonBareNames. This PR layers python-gated stack allowlists (mongo/celery/boto/aws/sqla). Residual: `cls`/`walk`/`remove`/`post`/`pop`/`offset`/`limit`/`exit`/`append` generic verbs (collision-prone), `now`/`utcnow` bare instance helpers | at-bar | one more wave for ≤1% (generic-verb allowlist with receiver-type gate) |
...
| client-fixture-a | python (Django backend, private fixture) | 4202 | 11.36% (2026-05-19, post python-stack-w5; was 12.56% pre-wave-5, 13.56% pre-#507/#521) | python-stack-w5 — cumulative delta: bug-extractor 3335 → 3021 (-314), bug-resolver 559 → 500 (-59), external-known 2456 → 2574 (+118). 4 diagnostic passes (pkg additions, bare-name additions, stdlib pkg residuals, docx). Chain-fixes #531 (external-sql disposition bucket), #532 (db:&lt;table&gt; entity synthesis), #526 (DRF ViewSet class-attr). | structural | Bulk of residual is python bare-other CALLS at the receiver-strip floor — addressable via #525 + #526 ViewSet class-attr. |
