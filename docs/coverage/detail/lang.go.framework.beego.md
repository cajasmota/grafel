...
| Request sink dataflow | 🟢 `partial` | `2026-06-04` | 3740 | — | #3872 extend-go-dataflow-sniffer-idioms: PARTIAL. dataflow_golang.go grew a beego arm in dfGoSourceFieldRe/dfGoSourceAnyRe recognising the controller-receiver static-key getters c.GetString("k") and c.Ctx.Input.Param("k") as request sources; a tainted read reaching a DB-write/response sink now fires request_sink_dataflow. Value-asserting tests: TestGoDataFlow_BeegoGetStringToDBCreate (field=name -> db.Create) and TestGoDataFlow_BeegoCtxInputParamToDBCreate (field=id -> db.Create). Honest-partial: c.ParseForm(&dto)/c.Input() whole-form binds not yet seeded. |
