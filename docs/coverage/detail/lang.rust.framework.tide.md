...
| Endpoint synthesis | ✅ `full` | `2026-06-03` | — | `internal/custom/rust/minor_fw_routing.go`<br>`internal/custom/rust/minor_fw_routing_test.go` | #3982: tide chained routes app.at("/p").get(a).post(b) — each verb on the same .at() now synthesises its own endpoint (previously only the first verb fired). Value-asserting test TestTideChainedVerbs asserts GET/POST /users -> list_users/create_user. |
| Handler attribution | ✅ `full` | `2026-06-03` | — | `internal/custom/rust/minor_fw_routing.go`<br>`internal/custom/rust/minor_fw_routing_test.go` | #3982: every verb in a tide chained route is attributed to its own handler. Asserted by TestTideChainedVerbs. |
