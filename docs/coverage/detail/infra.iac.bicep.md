...
| Iac output export extraction | ✅ `full` | `2026-06-04` | — | `internal/extractors/bicep/extractor.go` | Bicep `output <name> <type> = ...` declarations are extracted as SCOPE.Schema/output entities by `reOutput` (internal/extractors/bicep/extractor.go:80) + `extractOutputs` (internal/extractors/bicep/extractor.go:373), named by the output identifier — the values a Bicep module/template publishes to its caller. |
| Iac resource property extraction | 🟢 `partial` | `2026-06-03` | [link](https://github.com/cajasmota/grafel/issues/4199) | `internal/extractors/bicep/extractor.go` | Typed resource property-bag fields are stamped onto the resource entity Metadata: the deployed name: attribute (reNameAttr -> deployed_name) and the @<apiVersion> of the Azure type (api_version), plus resource_category, in `extractResources` (internal/extractors/bicep/extractor.go:177). Two genuine typed property-bag values are stamped per resource; the broader bag (sku/location/kind/properties{}) is not, so partial. |
