...
| Task routing | 🟢 `partial` | `2026-05-30` | [link](https://github.com/cajasmota/grafel/issues/3193) | `internal/custom/python/dramatiq.go`<br>`internal/custom/python/dramatiq_routing_test.go` | queue->actor routing extracted from @dramatiq.actor(queue_name=...) decorators and actor.send_with_options(queue_name=...) dispatch overrides. Partial because broker-level routing middleware (broker.add_middleware) and dynamic/computed queue names are not modelled. #6767 (2026-09-03) -- RE-DERIVED against what is emitted. ENTITY-ONLY, and deliberately so: both routing sites used to carry an inert `edge_kind: ROUTES_TO` PROPERTY while the pass emitted ZERO relationships. The property is removed and NO ROUTES_TO edge replaces it -- the send_with_options dispatch hop is already paired by a real ENQUEUES edge (internal/engine/scheduled_jobs_edges.go synthesizeDramatiqSendEdges), so a second edge would be the ADR-0028 section 3 double edge #6741 arm 4 refused for this pass, and the decorator form names no second node (the queue is a string, and the routing entity is already named after the actor). The routing fact lives on queue_name + actor/actor_ref; pinned by TestDramatiqTaskRoutingDoesNotClaimAnEdgeKind. |
