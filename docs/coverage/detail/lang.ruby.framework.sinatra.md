...
|------------|--------|-------------|-------|-------|-------|
| Endpoint deprecation versioning | ✅ `full` | `2026-06-03` | — | `internal/engine/http_endpoint_deprecation.go`<br>`internal/engine/http_endpoint_deprecation_ruby.go`<br>`internal/engine/http_endpoint_deprecation_ruby_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3628 Ruby port: deprecated/deprecation_source(+deprecated_since/deprecated_replacement)+path-derived api_version on synthesised Sinatra endpoints. rubyDeprecationVerdict reads the handler-decorator comment region above the `get '/x' do` block (StartLine-anchored): a YARD `# @deprecated <msg>` tag -> deprecated=true (source="# @deprecated", since/replacement parsed from the message); a `# Deprecated: <msg>` doc comment -> deprecated=true (source="# Deprecated:"). A Sunset/Deprecation response header in the block body -> deprecated=true (cross-language RFC-8594 path, source="Sunset response header"). api_version is path-derived from a /vN or /api/vN segment in the literal verb-block path. Value-asserted TestDeprecation_RubySinatraYardDeprecated (repl=/api/v2/users, api_version=1), TestDeprecation_RubySinceAndReplacement (since=2.0, repl=/reports/v2), TestDeprecation_RubyDeprecatedColonComment, TestDeprecation_RubySunsetResponseHeader; negatives TestDeprecation_RubyVersionlessNonDeprecated + the non-deprecated sibling in TestDeprecation_RubySinatraYardDeprecated. |
