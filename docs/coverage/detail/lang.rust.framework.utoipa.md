...
| Endpoint synthesis | 🟢 `partial` | `2026-08-25` | 6560 | `internal/custom/rust/helpers.go`<br>`internal/custom/rust/utoipa.go`<br>`internal/custom/rust/utoipa_test.go`<br>`internal/engine/http_endpoint_synthesis.go`<br>`internal/engine/http_endpoint_utoipa_axum.go`<br>`internal/engine/http_endpoint_utoipa_axum_test.go` | #6560 Arm A. TWO producers, neither of them complete. internal/custom/rust/utoipa.go:164-165 emits a SCOPE.Operation subtype=endpoint marker per #[utoipa::path] attribute (verb + canonical path + route_path prop; handler_name stamped at :174). That op is NOT a member of the canonical endpoint family: the endpoint enrichment passes gate on Kind == http_endpoint_definition and therefore skip it (applyEndpointPagination, internal/engine/http_endpoint_pagination.go:80; applyEndpointResponseCodes, internal/engine/http_endpoint_response_codes.go:100). Not inert, though — it carries the documented contract and merges by Name onto a same-file axum route op; see the Route extraction cell for that layer and its precondition. synthesizeUtoipaAxumRoutes (internal/engine/http_endpoint_utoipa_axum.go:261, dispatched at internal/engine/http_endpoint_synthesis.go:1350) is what mints the canonical http_endpoint_definition, and only for a SINGLE-handler routes!(handler) registration whose #[utoipa::path] sits in the SAME file, skipping any handler another producer already registers out of that file (utoipaRegisteredElsewhere, :232). PARTIAL, per the pass's own scope note at :66-79: multi-handler routes!(a, b) and path-qualified routes!(crate::items::list) do not match utoipaRoutesMacroRe (:102) and mint NOTHING (pinned by TestUtoipaAxum_MultiHandlerRoutesMacroNotMinted, internal/engine/http_endpoint_utoipa_axum_test.go:336); a handler whose attribute lives in another file is skipped, because the attribute map is built from one file only (:73-76, TestUtoipaAxum_UnknownHandlerNotMinted at :176); OpenApiRouter::nest prefix composition is not threaded onto these routes, so a nested router yields the UNPREFIXED attribute path (:77-79). Cross-file double mint is not deduped either (:54-58). #6560 Arms B1/B2 remain open. |
...
| Route extraction | 🟢 `partial` | `2026-08-25` | 6560 | `internal/custom/rust/helpers.go`<br>`internal/custom/rust/utoipa.go`<br>`internal/custom/rust/utoipa_test.go`<br>`internal/engine/http_endpoint_utoipa_axum.go`<br>`internal/engine/http_endpoint_utoipa_axum_test.go` | #6560 Arm A. utoipa::path(verb, path=...) yields a canonical verb+path: rustNormalizePath folds {id}/:id/<id> to the canonical {id} form (internal/custom/rust/utoipa.go:151), the decorated handler fn is captured from the fn following the attribute block (:158-162) and stamped as handler_name (:174), and the whole thing lands as a SCOPE.Operation subtype=endpoint op named 'METHOD path' (:164-165). TWO LAYERS, and they answer differently. At the SCOPE.Operation layer the old note's 'enriches bare axum/actix routes with documented contract' is REAL: axum.go:148-149 names its route op 'METHOD fullPath' with the identical Kind and subtype, and the house mechanism is merge-by-Name — MergeWithCustom keys on (SourceFile, Kind, Name) since #6104, as the sibling stampers state at internal/custom/rust/endpoint_response_codes.go:29 and :587 — so the attribute op merges onto the plain axum route op. PRECONDITION: it fires only when the two Names are identical, i.e. same source file AND the attribute path equal to the mount path. axum composes its nest prefix into fullPath (axum.go:143-144) while the attribute path stays unprefixed, so a nested or otherwise prefixed mount yields a different Name and does NOT merge. At the canonical http_endpoint_definition layer there is NO enrichment at all: synthesizeUtoipaAxumRoutes (internal/engine/http_endpoint_utoipa_axum.go:261) mints a SEPARATE definition and explicitly SKIPS any handler that synthesizeAxumRoutes or synthesizeRocket already registers from the same file (utoipaRegisteredElsewhere, :232) — a handler with a .route(...) mount is left alone there, not enriched. Both facts are true at their own layer; neither generalises to the other. PARTIAL: the registration forms the pass does not read are listed in its own header at :66-79 — multi-handler routes!(a, b) and path-qualified routes!(crate::items::list), both excluded by the single-identifier utoipaRoutesMacroRe at :102; a handler whose #[utoipa::path] lives in another file (:73-76); and OpenApiRouter::nest prefix composition, which leaves a nested router's endpoints on the unprefixed attribute path (:77-79). |
