...
| Endpoint synthesis | 🟢 `partial` | `2026-08-26` | 6560 | `internal/custom/rust/helpers.go`<br>`internal/custom/rust/utoipa.go`<br>`internal/custom/rust/utoipa_test.go`<br>`internal/engine/http_endpoint_synthesis.go`<br>`internal/engine/http_endpoint_utoipa_axum.go`<br>`internal/engine/http_endpoint_utoipa_axum_test.go` | #6560 Arms A+B1. TWO producers, neither of them complete. internal/custom/rust/utoipa.go:164-165 emits a SCOPE.Operation subtype=endpoint marker per #[utoipa::path] attribute (verb + canonical path + route_path prop; handler_name stamped at :174). That op is NOT a member of the canonical endpoint family: the endpoint enrichment passes gate on Kind == http_endpoint_definition and therefore skip it (applyEndpointPagination, internal/engine/http_endpoint_pagination.go:79; applyEndpointResponseCodes, internal/engine/http_endpoint_response_codes.go:100). Not inert, though — it carries the documented contract and merges by Name onto a same-file axum route op; see the Route extraction cell for that layer and its precondition. synthesizeUtoipaAxumRoutes (internal/engine/http_endpoint_utoipa_axum.go:314, dispatched at internal/engine/http_endpoint_synthesis.go:1350) is what mints the canonical http_endpoint_definition. ARM B1 LIFTED THE SINGLE-HANDLER RESTRICTION this cell used to record: utoipaRoutesMacroRe (:138-139) now matches routes!(a, b, c), utoipaMacroHandlerArgs (:144) splits the list, and each argument is resolved independently against the same-file attribute map (:336-348) — an unresolvable sibling is skipped WITHOUT suppressing the rest (TestUtoipaAxum_MixedResolvableAndImportedArguments, internal/engine/http_endpoint_utoipa_axum_test.go:724), and a handler named more than once, in one macro or across several, mints exactly once (the minted map at :326 and :353; TestUtoipaAxum_HandlerRepeatedAcrossMacrosMintedOnce at :550, TestUtoipaAxum_AdjacentMacrosNotSlurped at :586). Multi-handler minting is pinned positively by TestUtoipaAxum_MultiHandlerRoutesMacroMinted (:367) and its verb/path pairing by TestUtoipaAxum_MultiHandlerVerbsAndPathsNotSwapped (:406); these REPLACE TestUtoipaAxum_MultiHandlerRoutesMacroNotMinted, which this cell cited at :336 until B1 inverted it. Handlers another producer already registers out of the same file are still skipped (utoipaRegisteredElsewhere, :283; TestUtoipaAxum_MultiHandlerDedupeAgainstRouteCall at :508). STILL PARTIAL, per the pass's own scope note at :66-99, and the remaining gaps are Arm B2 rather than anything B1 changed: a path-qualified argument routes!(crate::items::list) fails the WHOLE macro match, single- or multi-handler (:125-130; tests at :461 and :486); a handler whose #[utoipa::path] lives in another file is skipped, because the attribute map is file-scoped (:86-89, TestUtoipaAxum_UnknownHandlerNotMinted at :177); OpenApiRouter::nest prefix composition is not threaded onto these routes, so a nested router yields the UNPREFIXED attribute path (:90-92); and cross-file double mint is deduped by neither the handler-name dedupe nor the emit-level dedupKey (:54-58). Four narrower shapes also fail to mint, recorded in the header at :94-105: a comment between arguments (routes!(a, /* x */ b)) kills the whole macro; a raw identifier (routes!(r#type)) never matches, since # is outside the identifier class; and routes ! ( a , b ) — whitespace before ! is Rust-legal — never matches, because the pattern requires routes! adjacent. Those three are misses in the safe direction and are pinned by TestUtoipaAxum_HeaderRecordedShapesMintNothing. The fourth is not: a macro whose NAME merely ends in routes! (my_routes!(a), app_routes!(a, b)) fails to match only because of the pattern's leading \b, and losing that word boundary would be PERMISSIVE — an ordinary user-defined wrapper macro would mint one phantom endpoint per argument (#6672, pinned by TestUtoipaAxum_PrefixedRoutesMacroMintsNothing). GRADE HELD AT PARTIAL, not raised: the sibling bar for Rust HTTP frameworks reserves partial for a statically-resolvable subset with an unresolved COMPOSITION form (lang.rust.framework.warp.md:21 is partial because .or() filter-tree composition is not attributable; lang.rust.framework.tower.md:21 and lang.rust.framework.hyper.md:21 likewise), and OpenApiRouter::nest composition plus cross-file handler resolution are exactly that. #6560 Arm B2 (B2a same-file nest, B2b cross-file resolution) remains open. |
