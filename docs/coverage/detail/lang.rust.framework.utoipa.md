...
| Route extraction | 🟢 `partial` | `2026-08-25` | 6560 | `internal/custom/rust/helpers.go`<br>`internal/custom/rust/utoipa.go`<br>`internal/custom/rust/utoipa_test.go`<br>`internal/engine/http_endpoint_utoipa_axum.go`<br>`internal/engine/http_endpoint_utoipa_axum_test.go` | #6560 Arm A. utoipa::path(verb, path=...) yields canonical verb+path (normalises {id}/:id/<id>) and captures the decorated handler fn (internal/custom/rust/utoipa.go:165-172). The former note claimed this 'enriches bare axum/actix routes with documented contract'; that is REMOVED because no code merges an attribute into a .route(...)-derived endpoint. What actually happens is a SEPARATE mint: synthesizeUtoipaAxumRoutes (internal/engine/http_endpoint_utoipa_axum.go:261) emits its own http_endpoint_definition, and explicitly SKIPS any handler that synthesizeAxumRoutes or synthesizeRocket already registers from the same file (utoipaRegisteredElsewhere, :232) — so a handler with a .route(...) mount is left alone rather than enriched. PARTIAL: the registration forms the pass does not read are listed in its own header at :66-79 — multi-handler routes!(a, b) and path-qualified routes!(crate::items::list), both excluded by the single-identifier utoipaRoutesMacroRe at :102; a handler whose #[utoipa::path] lives in another file (:73-76); and OpenApiRouter::nest prefix composition, which leaves a nested router's endpoints on the unprefixed attribute path (:77-79). |
