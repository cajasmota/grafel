...
| Request sink dataflow | 🟢 `partial` | `2026-06-04` | 3740 | — | #3872 extend-go-dataflow-sniffer-idioms: PARTIAL. dataflow_golang.go grew a go-zero bind arm in dfGoBindRe recognising httpx.Parse(r, &req) (the bound root is the SECOND arg); the populated struct becomes a request-derived root and its member access (req.Email) lifts the field at the sink. Value-asserting test TestGoDataFlow_GoZeroHttpxParseToDBCreate asserts field=Email -> db.Create. Honest-partial: only Parse(r, &x) shape; other httpx.Parse* variants share the arm via the .Parse( prefix. |
