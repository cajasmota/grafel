...
| Schema extraction | ✅ `full` | `2026-06-11` | 4714 | `internal/custom/python/dto_field_members.go`<br>`internal/custom/python/dto_field_members_test.go`<br>`internal/custom/python/marshmallow.go`<br>`internal/custom/python/testdata/marshmallow_nested.py`<br>`internal/patterns/schema_detector.go` | #4714: each marshmallow `class X(Schema)` declared field (name = fields.<T>(required=, allow_none=, load_default=, ...)) is emitted as a SCOPE.Schema/field member (field_name/field_type-from-fields.<T>/parent_class/optional/validators + parseable Signature) with a CONTAINS edge to the schema, the SAME uniform shape as the Pydantic/DRF (#4613) and JS (#4635) DTO field members. extractMarshmallowSchemaFields + emitPyDTOFieldMembers; required from required=True, optional from allow_none/load_default/missing. Value-asserted by TestMarshmallow_FieldMembers (type, optional, validators, CONTAINS). |
