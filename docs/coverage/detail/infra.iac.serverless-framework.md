...
| Iac resource property extraction | 🟢 `partial` | `2026-06-03` | [link](https://github.com/cajasmota/grafel/issues/4199) | `internal/engine/serverless_framework_edges.go` | Each function entity is stamped with its typed function-config properties: handler, runtime, region, service (parseProviderBlock + parseFunctionBody -> fnProps, internal/engine/serverless_framework_edges.go) — genuine per-function settings on the entity. memory/timeout/environment are not parsed/stamped (`slsFunction`, internal/engine/serverless_framework_parse.go:24-28, has only name/handler/events), so partial. |
