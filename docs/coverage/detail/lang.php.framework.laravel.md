...
|------------|--------|-------------|-------|-------|-------|
| Endpoint deprecation versioning | ✅ `full` | `2026-06-03` | 3628 | `internal/custom/php/deprecation_test.go`<br>`internal/custom/php/helpers.go`<br>`internal/custom/php/symfony.go`<br>`internal/engine/http_endpoint_deprecation.go` | #3628 PHP port: deprecated/deprecation_source(+deprecated_since/deprecated_replacement) stamped at the source on Symfony #[Route] SCOPE.Operation endpoints (symDeprecationNear), since they are custom-extractor entities the engine pass cannot reach. A `#[Route(..., deprecated: true)]` flag and a `@deprecated <msg>` PHPDoc above the action both fire, with the shared message parser pulling since/replacement. api_version is path-derived from a `/api/vN` route literal. Identical property contract to the flagship. |
...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| Auth coverage | ✅ `full` | `2026-06-11` | — | `internal/authposture/authposture_test.go`<br>`internal/authposture/laravel.go`<br>`internal/authposture/resolvers.go`<br>`internal/custom/php/laravel_authval.go` | Deep Laravel auth extraction: route middleware (auth/sanctum/api guards), Gate::define/authorize/policy, $this->authorize(), Policy classes+methods, @can/@cannot/@auth/@guest Blade directives, auth() helper, Auth:: facade, Sanctum/Passport HasApiTokens traits #4541 auth_posture_diff resolver (internal/authposture/laravel.go): decodes Laravel auth posture into the shared {kind,literal} vocabulary — ->middleware('auth'|'auth:api'|'auth:sanctum') and $this->middleware('auth') -> authenticated; ->middleware('role:admin') -> role; ->middleware('can:update,post')/'permission:x' and $this->authorize('x',$m)/Gate::allows -> action (Gate/Policy); superuser-named role -> superuser; ->withoutMiddleware('auth') / no auth middleware -> public. Per-route ▸ group/constructor precedence (->only()/->except() resolved by the engine); reconciled props win with a route/controller-source fallback. Fixture tests in authposture_test.go. #4752 ENGINE STAMPING: applyLaravelAuth (internal/engine/http_endpoint_php_auth.go) now reconciles route plus group middleware (auth/role:/can:/withoutMiddleware) into auth_required/auth_middleware/auth_roles/auth_permissions on the synthesized Laravel endpoints, so classic route/controller middleware decodes LIVE (structured), not only api-platform endpoints. Live-path tests: TestStamp_Laravel_RouteAuthMiddleware/GroupRoleMiddleware/WithoutMiddlewareOverride. |

### Validation

| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| DTO extraction | ✅ `full` | `2026-06-12` | — | `internal/custom/php/laravel_authval.go`<br>`internal/extractors/php/field_members.go`<br>`internal/extractors/php/issue4854_field_membership_test.go` | FormRequest subclasses extracted with per-field validation rules from rules() method body; authorize()/messages()/prepareForValidation hooks detected #4854: the framework/ORM-gated custom emitters only emitted field members for HTTP/ORM-bound DTOs; the GENERAL primary-pass now emits a SCOPE.Schema/field entity + class->field CONTAINS for EVERY typed property and promoted constructor parameter (Name '<Class>.<prop>', '$' stripped, dedups by Name in MergeWithCustom), plus an EXTENDS edge to an in-file parent class (interfaces excluded), so any PHP data class projects field rows in the dashboard shape tree — closing the same gap #4845/#4851 fixed for JS/TS and #4850/#4855 for Go. emitPhpFieldMembers + attachPhpExtends in php/field_members.go; value-asserted by TestPhpTypedPropertiesAndPromotedParamsAreContained/TestPhpBaseClassEmitsExtends. |
...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| Tests linkage | ✅ `full` | — | — | `internal/custom/php/phpunit_pest.go`<br>`internal/engine/http_endpoint_e2e_testmap.go`<br>`internal/extractors/cross/testmap/extractor_test.go`<br>`internal/extractors/cross/testmap/frameworks.go`<br>`internal/extractors/cross/testmap/resolver.go`<br>`internal/extractors/php/php.go`<br>`internal/extractors/php/tests.go`<br>`internal/graph/coverage.go` | Deep PHPUnit/Pest linkage (#3399): test* prefix + #[Test] attribute + @test docblock detection; class-name subject derivation (UserServiceTest->UserService); instantiation body hints (new Foo()); Pest it()/test() blocks with uses(Class::class) subject extraction and describe() PascalCase subject fallback; path hints for tests/ directory; TESTS edge for all three PHPUnit forms and Pest DSL. Receiver-typed test->CALLS->handler coverage (#4686): PHPUnit named test_x() methods carry local-variable receiver typing ($c = new XController($svc); $c->getCounts() => CALLS XController.getCounts), and Pest anonymous it()/test()/describe() closures get a per-file SCOPE.Operation/test_scope owner carrying the same receiver-typed CALLS, so ComputeCoverage credits test->CALLS->handler->endpoint. Laravel feature-test route-by-string hits ($this->get/getJson/postJson/json/call) populate e2e_route_calls => TESTS edge to the http_endpoint_definition via the shared resolve pass. Factory / app()->make() receivers stay bare (honest exclusion). |
...
| DB effect | 🟢 `partial` | `2026-06-11` | 4692 | `internal/links/effect_propagation.go`<br>`internal/substrate/effect_sinks_cross_orm_read_4692_test.go`<br>`internal/substrate/effect_sinks_php.go` | Eloquent/Doctrine/PDO call-shape detection (conf 0.85). #4692 read-reach: the Eloquent BUILDER/instance read form is now covered — ambiguous terminals (->where/->get/->first/->all/->pluck/->find/->value/->exists) credited db_read on a Model/Builder-typed $var (seeded from User::query()/->newQuery()/DB::table(), fixpoint over $q=$q->where(...) chains) and off an injected $this->model-> collaborator (phpEloquentBuilderReadMatches), so layered-repo reads ($this->model->where(...)->get()) reach. The static User::where(...) form and Doctrine ->find/->findBy stay bare; an ambiguous ->first()/->get() on a plain Collection local stays pure. |
...
| Request shape extraction | ✅ `full` | `2026-05-28` | [link](https://github.com/cajasmota/grafel/issues/2771) | `internal/links/payload_drift.go`<br>`internal/mcp/payload_drift_tool.go`<br>`internal/substrate/payload_shapes.go`<br>`internal/substrate/payload_shapes_php.go` | — |
...
| Response shape extraction | ✅ `full` | `2026-05-28` | [link](https://github.com/cajasmota/grafel/issues/2771) | `internal/links/payload_drift.go`<br>`internal/mcp/payload_drift_tool.go`<br>`internal/substrate/payload_shapes.go`<br>`internal/substrate/payload_shapes_php.go` | — |
| Sanitizer recognition | 🟢 `partial` | `2026-05-28` | — | `internal/links/taint_flow.go`<br>`internal/substrate/taint_sites_php.go` | — |
| Schema drift detection | ✅ `full` | `2026-05-28` | [link](https://github.com/cajasmota/grafel/issues/2771) | `internal/links/payload_drift.go`<br>`internal/mcp/payload_drift_tool.go`<br>`internal/substrate/payload_shapes.go`<br>`internal/substrate/payload_shapes_php.go` | — |
