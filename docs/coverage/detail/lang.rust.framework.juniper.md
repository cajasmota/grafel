...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| Type graph extraction | ✅ `full` | `2026-06-14` | — | `internal/custom/rust/juniper.go`<br>`internal/custom/rust/juniper_typegraph.go`<br>`internal/custom/rust/juniper_typegraph_test.go` | #5007: new internal/custom/rust/juniper_typegraph.go mirrors the async-graphql code-first type-graph extractor (#3983) for juniper. Emits SCOPE.Schema/type nodes (BuildOperationStructuralRef("graphql",file,Type), shared identity with the SDL/async-graphql/py/jsts passes) + GRAPH_RELATES field->type edges off #[derive(GraphQLObject)] struct fields and #[graphql_object]/#[graphql_subscription] impl resolver return types, carrying the SDL cardinality contract (field_name/list/nullable/item_nullable/cardinality/self_ref, framework=juniper). Probe TestJunTG_GraphQLObject_FieldGraph asserts User.orders Vec<Order> to_many + Option<Account> nullable to_one + manager Option<User> self_ref + scalar/GraphQLInputObject fields no edge; TestJunTG_ResolverReturnType asserts Query.user FieldResult<User> to_one unwrap + Vec<Order> to_many; wrong-language + plain-struct no-op probes. #5109 DEEPENS along three axes: (a) #[graphql(name="...")] field/type rename is honoured — the GraphQL name (not the Rust ident) keys graphql_type/graphql_field/field_name AND the node/edge structural identity (TestJunTG_FieldAndTypeRename: UserAccount->Account, orders->primaryOrders); (b) cross-file targets — a capitalized non-scalar field/return type not declared same-file now emits a GRAPH_RELATES edge with a by-name ToID (Kind:SCOPE.Schema:Type) the resolver binds via its global by-name index, carrying cross_file=true (TestJunTG_CrossFileEdge: User.external Vec<RemoteThing>; lowercase non-type token still no edge); (c) #[graphql_interface(for=[A,B])] traits — interface type node (graphql_kind=interface) + GRAPH_RELATES Object->Interface edges with relation=implements (both the for=[...] list and impl Trait for Object forms), and a field typed as the interface trait is a recognized edge target (TestJunTG_Interface). Honest limit: cross-file is by-NAME (ambiguous names dropped, no false edge); GraphQLInputObject/GraphQLEnum stay DTO-catalog (#4964). |
