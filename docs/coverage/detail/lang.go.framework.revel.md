...
| Request sink dataflow | 🟢 `partial` | `2026-06-04` | 3740 | — | #3872 extend-go-dataflow-sniffer-idioms: PARTIAL. dataflow_golang.go grew a revel arm recognising the controller key getter c.Params.Get("k") (static key) in dfGoSourceFieldRe and the bind helper c.Params.Bind(&x, "k") in dfGoBindRe. Value-asserting tests: TestGoDataFlow_RevelParamsGetToDBCreate (field=id -> db.Create) and TestGoDataFlow_RevelParamsBindToDBSave (field=Name lifted via the Bind root -> db.Save). Honest-partial: c.Params.Values (whole map) not seeded. |
