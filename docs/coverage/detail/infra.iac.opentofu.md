...
| Iac cross stack reference | ✅ `full` | `2026-06-04` | — | `internal/extractors/hcl` | Full parity with Terraform: `data.terraform_remote_state.*.outputs.*` in .tofu files emits the same cross-stack DEPENDS_ON edge (cross_stack=true) via `extractRemoteStateDeps` (internal/extractors/hcl/terraform_deep.go:221-260), since entities carry Language="terraform". |
| Iac environment region account | — `not_applicable` | — | — | — | OpenTofu (shared hcl extractor) sets region inside the `provider` block (e.g. region = "us-east-1") and account/state via `backend`; `extractProviderBlock` (internal/extractors/hcl/extractor.go:435) records only the provider *name* label — it never reads the region attribute — and terraform_deep.go captures only the backend *type* label (internal/extractors/hcl/terraform_deep.go), so no region/account/runtime value is stamped as an environment-targeting property. Honest-missing. |
...
| Iac output export extraction | ✅ `full` | `2026-06-04` | — | `internal/extractors/hcl` | Full parity with Terraform: `output "name" {}` blocks in .tofu files flow through the same `extractOutputBlock` (internal/extractors/hcl/extractor.go:410) as .tf because the classifier routes .tofu to the shared "terraform" language token, emitting SCOPE.Schema/output entities identically. |
| Iac resource property extraction | 🟢 `partial` | `2026-06-03` | 4229 | `internal/extractors/hcl/extractor.go`<br>`internal/extractors/hcl/resource_properties.go` | Epic #4194: OpenTofu shares the hcl/terraform tree-sitter extractor, so .tofu/.tofu.json get the SAME curated scalar property stamping as .tf with zero extra code. stampResourceScalarProperties (resource_properties.go) stamps the curated allow-list (instance_type/runtime/memory_size/timeout/count/engine/port/...) of literal scalars (string_lit/numeric_lit/bool_lit) onto resource Properties; references/interpolations/collections are skipped (stay edges). Same value-asserting tests as terraform (resource_properties_test.go). PARTIAL: curated subset by design. DEPLOY-DEFERRED. |
| Iac stack app topology | ✅ `full` | `2026-06-04` | [link](https://github.com/cajasmota/grafel/issues/4200) | `internal/extractors/hcl/extractor.go`<br>`internal/extractors/hcl/relationships.go` | Full parity with Terraform (shared hcl extractor): each `module "<name>"` block in .tofu files is emitted as a SCOPE.Component / subtype=module composition entity (`extractModuleBlock`, internal/extractors/hcl/extractor.go:329) plus a file→module IMPORTS containment edge (import_kind=module, internal/extractors/hcl/relationships.go). Full for the HCL `module` composition idiom. |
