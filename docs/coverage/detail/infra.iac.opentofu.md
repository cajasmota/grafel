...
| Iac cross stack reference | ✅ `full` | `2026-06-04` | — | `internal/extractors/hcl` | Full parity with Terraform: `data.terraform_remote_state.*.outputs.*` in .tofu files emits the same cross-stack DEPENDS_ON edge (cross_stack=true) via `extractRemoteStateDeps` (internal/extractors/hcl/terraform_deep.go:220-260), since entities carry Language="terraform". |
| Iac environment region account | — `not_applicable` | — | — | — | OpenTofu (shared hcl extractor) sets region inside the `provider` block (e.g. region = "us-east-1") and account/state via `backend`; `extractProviderBlock` (internal/extractors/hcl/extractor.go:457) records only the provider *name* label — it never reads the region attribute — and terraform_deep.go captures only the backend *type* label (internal/extractors/hcl/terraform_deep.go), so no region/account/runtime value is stamped as an environment-targeting property. Honest-missing. |
