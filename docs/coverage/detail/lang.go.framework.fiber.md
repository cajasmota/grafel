...
| Rate limit stamping | ✅ `full` | `2026-06-03` | [link](https://github.com/cajasmota/archigraph/issues/4086) | `internal/custom/golang/fiber.go`<br>`internal/custom/golang/rate_limit_route.go`<br>`internal/custom/golang/rate_limit_route_test.go` | tollbooth / ulule-limiter / golang.org/x/time/rate: a limiter applied as inline-route / group / engine-wide middleware stamps rate_limited/rate_limit_scope(route|group|engine)/rate_limit_source on the route op; a rate.NewLimiter(rate.Limit(N),burst) or tollbooth NewLimiter(N)/SetMax(N) literal resolves rate="N/s". Imported/ulule/config-driven limiters → rate_limited=true with rate omitted (honest-partial). Negative: a limiter constructed but never applied, and a non-limiter middleware (logger), are not stamped. Out of scope: the in-handler `if !limiter.Allow()` guard (not middleware-applied) and redis_rate per-period rates. |
