...
| Request sink dataflow | ✅ `full` | `2026-06-02` | 3902 | `internal/links/dataflow_pass.go`<br>`internal/substrate/dataflow.go`<br>`internal/substrate/dataflow_jsts.go` | NestJS request-input → sink DATA_FLOWS_TO (#3902, epic #3872): the JS/TS dataflow sniffer now recognises NestJS controller-method parameters injected by @Body()/@Query()/@Param()/@Headers()/@Req()/@Request() as request-derived SOURCES — the same role req.body plays in Express. dataflow_jsts.go: (a) jstsNestJSHeaders/jstsAllHeaders recover NestJS method headers the shared jstsFuncHeaderRe drops (its param list excludes inner parens, so create(@Body() dto) was invisible); (b) at handler entry jstsNestJSParamTaints seeds each decorated param as a tainted root, field = the decorator literal key when present (@Param('id') id → field id); (c) the field is otherwise recovered from a static member access off the param (@Body() dto → dto.email → field email). Reuses the existing sink set (ORM create/save/insert, res.json/send, axios/fetch) and the full multi-hop + cross-file boundary machinery (DataFlowMaxHops=3, continueDataFlowJSTS). HONEST-PARTIAL unchanged: drops reassignment, branch-merge, collection mutation, dynamic keys, embedded-arg, spread/rest/destructured params, recursion/entity-cycle, the 4th hop, external/unresolved imports; a constructor-injected (non-request) param is NOT a source. Tests: dataflow_jsts_nestjs_test.go (@Body→repo.save field=email; @Query('q')→res.json field=q; @Param('id')→repo.create field=id; multi-line signature; one-hop pass-through; cross-file boundary; injected-service & static-value negatives). DEPLOY-DEFERRED (daemon not rebuilt). |
