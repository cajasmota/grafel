...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| DTO extraction | ✅ `full` | `2026-06-02` | — | `internal/custom/javascript/nestjs.go`<br>`internal/custom/javascript/reqresp_dto_test.go`<br>`internal/custom/javascript/validation_schema.go`<br>`internal/custom/javascript/validation_schema_test.go` | nestjs.go emits endpoint→DTO edges (#3629/#3607): each route SCOPE.Operation carries ACCEPTS_INPUT → Class:<Dto> for @Body()-typed params and RETURNS → Class:<Ret> for the typed return. Complementing that, validation_schema.go now extracts the class-validator DTO body itself: a class with @IsString/@IsInt/@IsEmail/... decorators is emitted as a SCOPE.Schema entity carrying captured field names+types (field_<name> props; TS annotation refines, else decorator type). Plain classes with no class-validator decorators are skipped (no false schema). Tests: TestClassValidator_FieldsFromDecorators, TestClassValidator_PlainClassSkipped. |
