...
| Tanstack query extraction | ✅ `full` | `2026-06-24` | [link](https://github.com/cajasmota/grafel/issues/5492) | `internal/extractors/javascript/issue5492_tanstack_entities_test.go`<br>`internal/extractors/javascript/react.go`<br>`internal/extractors/javascript/react_ecosystem.go`<br>`internal/extractors/javascript/testdata/react_ecosystem/TanstackEntities.tsx` | TanStack/React Query hook calls are extracted as ENTITIES (#5492): each useQuery/useSuspenseQuery/useInfiniteQuery/useQueries inside a component/custom-hook body is a decorated SCOPE.Operation subtype=tanstack_query, and useMutation is subtype=tanstack_mutation (via=tanstack_query, framework=react), with a CONTAINS edge from the enclosing component/hook. Attributes captured: query_key (literal queryKey array stringified, e.g. ['user', id] -> 'user,id'), query_fn / mutation_fn (the queryFn/mutationFn callee ref name when resolvable), query_kind, query_call. Handles both the object-arg form (useQuery({queryKey, queryFn})) and the older positional form (useQuery(key, fn)). Import-gated on @tanstack/react-query (or react-query) so a non-TanStack useQuery is not mis-tagged. The queryKey->endpoint USES edge is the follow-up #5494. useQueryClient still surfaces as a plain USES_HOOK edge (it returns the client handle, not an operation). Real-world: TanStack/query react examples 42 useQuery, 6 useMutation, 3 useInfiniteQuery, 7 useSuspenseQuery. |
