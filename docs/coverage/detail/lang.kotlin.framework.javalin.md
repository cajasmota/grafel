...
| Endpoint synthesis | ✅ `full` | `2026-06-03` | — | `internal/custom/kotlin/javalin_routes.go`<br>`internal/custom/kotlin/javalin_routes_test.go` | #4017 (epic #3872): native custom_kotlin_javalin_routes extractor (the Java javalin extractor only runs via custom_java_patterns on .java files, so Javalin-Kotlin synthesized zero before this). Emits a SCOPE.Operation `endpoint` entity named '<VERB> <path>' (http_method/path/framework/provenance=INFERRED_FROM_JAVALIN_ROUTE) per route — the SAME entity shape the http4k/spring/micronaut/quarkus Kotlin route extractors emit and that the graph treats as the synthesized endpoint. Dispatches live on .kt via CustomExtractorsFor(kotlin) (TestEveryRegisteredCustomKeyDispatches green). Value-asserted: GET/POST /users, GET/DELETE /users/{id} from direct DSL; composed GET /users + DELETE /users/{id} from ApiBuilder path() nesting (TestJavalinRoutes_DirectLambda/_DirectMethodRef/_ChainedCreate/_ApiBuilderNested/_ApiBuilderSubPathArg). |
| Handler attribution | 🟢 `partial` | `2026-06-03` | — | `internal/custom/kotlin/javalin_routes.go`<br>`internal/custom/kotlin/javalin_routes_test.go` | #4017 (epic #3872): handler stamped on the endpoint entity's `handler` property. Method references (::createUser, UserController::getOne) and constructed handlers (UserHandler() -> UserHandler) are attributed verbatim; trailing/inline lambdas are attributed as 'lambda' (value-asserted in TestJavalinRoutes_DirectMethodRef/_ApiBuilderNested/_ApiBuilderSubPathArg). Honest-partial: the custom-extractor Extract() interface returns entities only (no RelationshipRecord), so there is no ROUTES_TO/HANDLES edge to a named handler entity, and an inline-lambda body is not resolved to an enclosing fun — same edge-less attribution model as the sibling http4k/spring Kotlin route extractors. |
| Route extraction | ✅ `full` | `2026-06-03` | — | `internal/custom/kotlin/javalin_routes.go`<br>`internal/custom/kotlin/javalin_routes_test.go` | #4017 (epic #3872): native custom_kotlin_javalin_routes extracts both Javalin route idioms in Kotlin. Direct fluent DSL: <receiver>.get/post/put/delete/patch/head/options("path"){...} incl. chained Javalin.create().get(...). ApiBuilder DSL: app.routes{ path("p"){ get(::h); post(...) } } with brace-depth prefix-stack composition (path() nesting) mirroring the http4k nested-bind scanner. Negatives: app.config(...)/app.start(...)/before/after emit nothing (TestJavalinRoutes_NonRouteCalls); a get/post file with no Javalin signal no-ops (TestJavalinRoutes_NoJavalinSignal) so it never poaches http4k/ktor routes. Value-asserted GET/POST/PUT/DELETE verb+path in TestJavalinRoutes_*. |
