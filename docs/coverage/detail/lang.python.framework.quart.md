...
| Endpoint response codes | ✅ `full` | `2026-06-03` | — | `internal/engine/http_endpoint_response_codes.go`<br>`internal/engine/http_endpoint_response_codes_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3913: applyEndpointResponseCodes resolves quart JSONResponse(status_code=NNN) and the Flask-shaped tuple-return idiom 'return body, NNN[, headers]' (paren-balance guarded so call-arg ints are not mis-read); honest-partial on dynamic status vars. |
