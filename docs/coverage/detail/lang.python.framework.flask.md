...
| Endpoint pagination posture | 🟢 `partial` | `2026-06-03` | 3872 | `internal/engine/http_endpoint_pagination.go`<br>`internal/engine/http_endpoint_pagination_patterns.go`<br>`internal/engine/http_endpoint_pagination_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3628/#3872: applyEndpointPagination stamps paginated/pagination_style/pagination_params on the flask handler via pythonPaginationVerdict/pythonRequestQueryParams — limit+offset/page/cursor read from request.args.get(...) in the handler body (classifyParamShape). Partial: request.args-read shape only. Honest-partial: a lone limit is ambiguous and left unstamped. |
...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| View rendering | ✅ `full` | `2026-06-02` | 3628 | `internal/extractor/template_render.go`<br>`internal/extractors/python/template_render.go`<br>`internal/extractors/python/template_render_test.go` | render_template('x.html') -> RENDERS SCOPE.Template; dynamic/f-string names dropped (#3628) |

### Auth

| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| Auth coverage | ✅ `full` | `2026-06-11` | 3052 | `internal/authposture/authposture_test.go`<br>`internal/authposture/flask.go`<br>`internal/authposture/resolvers.go`<br>`internal/custom/python/auth_endpoint.go`<br>`internal/custom/python/auth_endpoint_test.go`<br>`internal/custom/python/flask.go`<br>`internal/mcp/auth_coverage.go` | #3628 area #6 (endpoint protection): flask.go now scans the stacked-decorator block of each @app.route / @app.get route for a Flask-Login / Flask-Security / Flask-Principal auth decorator (login_required/roles_required/permission_required/jwt_required/...), stamping auth_required=true/auth_method=decorator/auth_confidence/auth_guard ON THE ENDPOINT entity, with roles captured from @roles_required('admin','superuser') (resolveFlaskDecoratorAuth in auth_endpoint.go). Value-asserting tests: TestFlaskAuth_LoginRequired (/dashboard -> auth_required=true, auth_guard=login_required, auth_method=decorator), TestFlaskAuth_RolesRequired (auth_roles=admin,superuser), negative TestFlaskAuth_NoDecoratorUnprotected (/public -> no auth_required). #4540 auth_posture_diff resolver (internal/authposture/flask.go): decodes Flask auth posture into the shared {kind,literal} vocabulary — @login_required/@requires_auth -> authenticated; @roles_required('admin') -> role; @permission_required('export') -> action; @admin_required/@superuser_required -> role/superuser; @app/@bp.before_request hook -> authenticated at the blueprint/app scope; no decorator + no before_request -> public/unknown (honest). View-decorator ▸ before_request precedence; reconciled props win with a view-source fallback. Fixture tests in authposture_test.go. #4752 ENGINE STAMPING: flask.go and auth_endpoint.go now also stamp auth_decorator plus auth_page (permission alias) and view_source, and recognise @admin_required/@superuser_required by name, so role/permission/admin Flask postures decode LIVE (structured). Live-path tests: TestFlaskAuth_RolesRequired/PermissionRequired/AdminDecorator_4752. |
