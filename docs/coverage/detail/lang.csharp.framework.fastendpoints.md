...
| Request sink dataflow | 🟢 `partial` | `2026-06-04` | 4164 | `internal/links/dataflow_pass.go`<br>`internal/substrate/dataflow.go`<br>`internal/substrate/dataflow_csharp.go`<br>`internal/substrate/dataflow_csharp_test.go` | SCOPED request-input → sink DATA_FLOWS_TO (#3628 area #22, epic #3872, #4164): the C# sniffer (internal/substrate/dataflow_csharp.go) now seeds the FastEndpoints typed request DTO as a request-derived root — in a file carrying a FastEndpoints signal (dfCSharpFastEndpointSignalRe: `using FastEndpoints` or `: Endpoint<`), the FIRST parameter of the fixed handler method HandleAsync/ExecuteAsync (dfCSharpFEHandlerRe) is a whole-object source (field "", recovered from req.Property access, mirroring [FromBody]); a CancellationToken-only parameter (EndpointWithoutRequest) is never seeded (csharpFastEndpointReqTaint). Reuses the existing ASP.NET intra-method taint tracking, multi-hop (<=DataFlowMaxHops=3) local propagation, cross-file boundary emission (continueDataFlowCSharp), and the shared sink table: EF Core write (_context.Set.Add/AddRange/Update/Remove, SaveChanges/SaveChangesAsync, ExecuteSqlRaw), Dapper (connection.Execute/ExecuteAsync), response (Ok/Json/Content/return <tainted>), outbound HTTP (httpClient.PostAsync/SendAsync/PostAsJsonAsync). HONEST-PARTIAL: inherits the ASP.NET sniffer's drops (static/constant values, injected services, reassignment, embedded-arg expressions, params arrays, recursion/cycle, the 4th hop, external/unresolved imports); whole-object req without a property access flows with field ""; FastEndpoints' BindAsync custom binders and [FromHeader]-style attribute properties on the request DTO are not field-resolved. DEPLOY-DEFERRED (daemon not rebuilt). |
