...
| Endpoint pagination posture | ✅ `full` | `2026-06-03` | — | `internal/engine/http_endpoint_pagination.go`<br>`internal/engine/http_endpoint_pagination_patterns.go`<br>`internal/engine/http_endpoint_pagination_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3913: applyEndpointPagination stamps paginated/pagination_style/pagination_params from typed signature params (page/page_size/limit/offset/cursor) plus literal-named request.query_params reads in the handler body and the cross-language parameters fallback. Honest-partial: a lone limit and dynamically-named reads are not stamped. |
| Endpoint response codes | ✅ `full` | `2026-06-03` | — | `internal/engine/http_endpoint_response_codes.go`<br>`internal/engine/http_endpoint_response_codes_test.go`<br>`internal/engine/http_endpoint_synthesis.go` | #3913: applyEndpointResponseCodes resolves litestar @get/@post(status_code=NNN) decorator codes plus JSONResponse/Response(status_code=NNN) body literals; honest-partial on dynamic status vars. |
