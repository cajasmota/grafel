...
| Endpoint synthesis | ✅ `full` | `2026-06-11` | — | `internal/engine/go_routes.go`<br>`internal/engine/http_endpoint_go_group_prefix_4408_test.go`<br>`internal/engine/response_shape_go.go`<br>`internal/engine/rules/go/frameworks/echo.yaml` | #4408: synthesizeGoRouters (response_shape_go.go) resolves echo `e.Group("/api")` route-group prefixes (same `.Group(` spelling/mechanism as gin) and prepends them to routes registered on the group variable, including nested groups, via goGroupPrefixIndex. Value-asserting test TestGroupPrefix4408_EchoGroup. Honest-partial: non-literal/setup-func-passed groups fall back to the route's own path. |
...
| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| Auth coverage | ✅ `full` | `2026-06-11` | — | `internal/authposture/authposture_test.go`<br>`internal/authposture/gomiddleware.go`<br>`internal/authposture/resolvers.go`<br>`internal/custom/golang/helpers.go`<br>`internal/custom/golang/middleware_auth_test.go`<br>`internal/custom/golang/route_auth.go`<br>`internal/custom/golang/route_auth_test.go` | Two layers. (1) #3734 endpoint-protection stamping (route_auth.go): binds auth middleware to the route SCOPE.Operation op and stamps the #3696 flat contract (auth_required/auth_method=middleware/auth_guard/auth_kind/auth_confidence). Echo passes trailing middleware after the handler (e.GET('/admin', h, jwtMiddleware) -> /admin auth_required, kind=jwt, HIGH), group-level (g := e.Group('/admin', jwtMiddleware) -> routes on g auth_required, HIGH), engine-wide .Use() -> MEDIUM. Value-asserting tests TestEchoTrailingMiddlewareAuth/TestEchoGroupAuth + negatives (/public,/login not stamped). (2) #3213 .Use() chain pattern catalog (jwt/oauth/basic/session/rbac/api_key/auth). Honest-partial: dynamic/conditional middleware and roles/scopes from opaque guards not modelled. #4543 auth_posture_diff resolver (internal/authposture/gomiddleware.go): decodes Go HTTP middleware-chain auth posture (Gin/Echo/Chi/net-http) into the shared {kind,literal} vocabulary across router-level r.Use(AuthMiddleware()), group-level g.Use(jwtAuth())/chi r.Group(func(r){ r.Use(Auth) }), inline route r.GET('/x', AuthRequired(), h), and net/http AuthMiddleware(handler) wrappers — an auth-ish middleware name (auth/jwt/authenticate/requireAuth/bearer/session) reaching a route -> authenticated; RequireRole('admin')/RequirePermission('x') -> role/action; superuser-named role/RequireAdmin -> superuser. HONEST name-heuristic limit (Go middleware carries no standard auth annotation; auth-ness is by identifier): a route reaching no recognisable auth middleware with no explicit public marker -> unknown (under-claim, never false-public); auth_required=false / an exported Public/NoAuth/AllowAnonymous middleware -> public. Reconciled props (auth_middleware/auth_roles/auth_permissions when stamped) win with a route/handler-source fallback. Fixture tests in authposture_test.go (router-Use/group/inline/role/no-auth-unknown/explicit-public/E2E-looser-vs-Django-oracle). |

### Validation

| Capability | Status | Verified at | Issue | Cites | Notes |
|------------|--------|-------------|-------|-------|-------|
| DTO extraction | ✅ `full` | `2026-06-11` | — | `internal/custom/golang/dto.go`<br>`internal/custom/golang/dto_edges_test.go`<br>`internal/custom/golang/dto_field_members.go`<br>`internal/custom/golang/dto_field_members_test.go` | #4715: each request/response DTO struct field is emitted as a SCOPE.Schema/field member (field_name from json tag, normalized field_type, parent_class, optional from omitempty/pointer/non-required validate|binding rule, validators as @rule markers + parseable Signature) with a CONTAINS edge to the struct — the SAME uniform shape as the JS (#4635) and Python/Java (#4613) DTO field members. emitGoDTOFieldMembers in dto_field_members.go; value-asserted by TestGoDTO_FieldMembers (type/optional/validators/CONTAINS). DTO struct resolution stays same-file heuristic (partial where not proven by a per-framework fixture). |
