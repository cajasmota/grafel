...
|---------|---------|------|--------------------------|
| `GRAFEL_EXTRACT_GOMAXPROCS` | `1` | background (opt-in `GRAFEL_SUBPROC_EXTRACT` only) | `GOMAXPROCS` set on each **background** extract subprocess, and (since [#5960](https://github.com/cajasmota/grafel/issues/5960)) the hard cap on that child's concurrent tree-sitter parses. Total background extract draw = `concurrency × this`, held at or below the core budget (25% of host cores). Default dropped `2` → `1` in #5960: each child extracts its batch sequentially, so the budget buys more by running more children than by giving one child more `GOMAXPROCS`. Raising this shrinks the auto-tuned `concurrency` proportionally so the budget still holds. |
...
| `GRAFEL_EXTRACT_CONCURRENCY` | auto (background: core budget `max(1, NumCPU/4)` ÷ `GRAFEL_EXTRACT_GOMAXPROCS` → 3 on a 12-core host, 8 on a 32-core host; explicit: `NumCPU`) | both (opt-in `GRAFEL_SUBPROC_EXTRACT` only) | Hard ceiling on the number of concurrent extract subprocesses, honored on **both** paths. Set to `1` as an emergency throttle on a contended machine — it caps explicit rebuilds too. |
...
| `GRAFEL_SUBPROCESS_INDEXER` | `1` (on) | background | Runs each reindex job as a short-lived child process (`grafel index-internal`) bounded to the daemon-wide reindex ceiling (`ReindexGraphPhaseGOMAXPROCS`, [#5602](https://github.com/cajasmota/grafel/issues/5602)) — **not** `GRAFEL_EXTRACT_GOMAXPROCS`, which only applies on the opt-in `GRAFEL_SUBPROC_EXTRACT` fanout — instead of indexing in-process at the daemon's `GOMAXPROCS`. **Resource-safe default (v0.1.1):** keeps the daemon heap flat AND bounds per-reindex CPU so background work can't saturate the host. Set to `0`/`false` to force the legacy in-process path. |
