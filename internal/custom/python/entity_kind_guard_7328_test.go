...
// could be absent from entity, or present and unreachable, and every other
...
// internal/custom/python/helpers.go fails HERE and not only somewhere downstream.
//
...
func TestProducedEntityKindGuard7328_Python(t *testing.T) {
	t.Run("undeclared kind panics", func(t *testing.T) {
		defer func() {
			r := recover()
			if r == nil {
				t.Fatalf("entity accepted the undeclared kind %q: the "+
					"types.ValidateProducedEntityKind call in internal/custom/python is missing or "+
		...
		_ = entity("N", undeclaredKind7328, "sub", "f.py", 1, map[string]string{})
	})

	t.Run("declared kind does not panic", func(t *testing.T) {
		defer func() {
			if r := recover(); r != nil {
				t.Fatalf("entity rejected the declared kind %q: %v",
					string(types.EntityKindClass), r)
			}
		}()
		got := entity("N", string(types.EntityKindClass), "sub", "f.py", 1, map[string]string{})
	...
	t.Run("recorded undeclared kind does not panic", func(t *testing.T) {
		...
		defer func() {
			if r := recover(); r != nil {
				t.Fatalf("entity rejected the recorded kind %q: %v", recordedKind7328, r)
			}
		}()
		got := entity("N", recordedKind7328, "sub", "f.py", 1, map[string]string{})
