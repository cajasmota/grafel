...
// internal/custom/rust/helpers.go fails HERE and not only somewhere downstream.
//
...
func TestProducedEntityKindGuard7328_Rust(t *testing.T) {
	t.Run("undeclared kind panics", func(t *testing.T) {
		defer func() {
			r := recover()
			if r == nil {
				t.Fatalf("makeEntity accepted the undeclared kind %q: the "+
					"types.ValidateProducedEntityKind call in internal/custom/rust is missing or "+
