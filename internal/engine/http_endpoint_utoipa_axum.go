...
// and a macro whose NAME merely ENDS in `routes!` (`my_routes!(a)`,
// `app_routes!(a, b)`) never matches, because the pattern's leading `\b`
// requires a word boundary before `routes`.
//
// The first three are misses in the safe direction and none is a regression
// from Arm A. The fourth is the opposite: losing that word boundary would be
// PERMISSIVE — an ordinary user-defined wrapper macro would mint one phantom
// endpoint per argument for a registration this pass never saw. All four are
// pinned (TestUtoipaAxum_HeaderRecordedShapesMintNothing,
