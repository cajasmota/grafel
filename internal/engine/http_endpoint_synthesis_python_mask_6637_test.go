...
// direction #6624 pinned. The pristine predicate is UNCONDITIONAL, so this
// family has no strictly-more-permissive member to score first: every mutant
// in it narrows what counts as an escape, and M1/M4 are the two narrowings
// that survive #6624's two fixtures.
//
// BRANCH ATTRIBUTION, measured rather than argued. M1's exact conditional was
// also applied to the TWIN skip at :3202-3205 (M7). It compiles, and the full
// package — this row included — passes at exit 0. So this row fires on the
// single-line skip and on nothing else; the twin is invisible to it, which is
// what "the :3226 branch, not the :3202 one" has to mean to be worth writing
// down. (That the twin survives the whole suite is a separate gap on the
// triple-quoted branch and is NOT claimed or fixed here.)
//
// RECORDED EQUIVALENT — do not re-score as DEAD. Adding a bounds guard alone,
// `if b[j] == '\\' && j+1 < len(b)`, is equivalent: it differs from pristine
// only on a trailing backslash at end of buffer, where pristine sets
// j = len(b)+1 and clamps to len(b) while the mutant falls through to `j++`
// and reaches len(b) — both leave the literal unterminated and blank the same
// span. Measured: identical output on all 5,380,840 inputs of the sweep space
// below.
//
// FIXTURE and GUARD, brute forced rather than hand-picked: every string of
// length <= 7 over {x, ' ', '=', '"', '\”, '#', '\\', 'a', '\n'} was scored
// for guard-pass x survival of M1 and M4. 8336 inputs satisfy the guard below
// and NONE of them leaves either mutant alive, so no edit to this fixture that
// still satisfies its own premise can make the row vacuous. Each clause is
// annotated with the number of survivors admitted by dropping THAT clause
// alone.
//
...
func TestPythonMaskInertRegions_EscapedNewlineContinuesLiteral_6637(t *testing.T) {
	...
	// c0 LOAD-BEARING (4166 survivors admitted if dropped). The scanner
	...
	// c1 DECORATIVE (0 survivors admitted; c2 and c6 already reject every such
	// fixture in the sweep space). Kept because it names the branch under
	// test: the single-line scan runs only when the triple-quote probe fails.
	...
	// c2 BRANCH IDENTITY, not a kill guard (0 survivors admitted if dropped —
	...
	// c4 LOAD-BEARING (1516). Containing a backslash is not enough: the scan
	...
	// c5 LOAD-BEARING (66642), and the clause that distinguishes this row from
	...
	// c6 LOAD-BEARING (1120). The literal must TERMINATE on the continuation
	...
	// end up at the same output — a fixture that passes its guard and observes
	// nothing. It is also what makes the literal terminated, so #6418 says the
	// pristine output is the input unchanged.
	...
	// c7 DECORATIVE (0 survivors admitted). Nothing after the closing quote may
