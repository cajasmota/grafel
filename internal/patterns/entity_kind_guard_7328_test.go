...
// internal/patterns/helpers.go fails HERE and not only somewhere downstream.
//
...
func TestProducedEntityKindGuard7328_Patterns(t *testing.T) {
	t.Run("undeclared kind panics", func(t *testing.T) {
		defer func() {
			r := recover()
			if r == nil {
				t.Fatalf("makeEntity accepted the undeclared kind %q: the "+
					"types.ValidateProducedEntityKind call in internal/patterns is missing or "+
		...
		_ = makeEntity("f.src", "N", undeclaredKind7328, "sub", "lang", 1, map[string]string{})
	})
	...
	t.Run("declared kind does not panic", func(t *testing.T) {
		...
		got := makeEntity("f.src", "N", string(types.EntityKindClass), "sub", "lang", 1, map[string]string{})
	...
	t.Run("recorded undeclared kind does not panic", func(t *testing.T) {
		...
		got := makeEntity("f.src", "N", recordedKind7328, "sub", "lang", 1, map[string]string{})
