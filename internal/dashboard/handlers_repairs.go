...
func (s *Server) handleListFindings(w http.ResponseWriter, r *http.Request) {
	...
	// #6178 round 3: save_finding output is hand-curated, not regenerated by
	// any pass, so a GRAFEL_HOME user whose findings look empty here gets a
	// signal rather than silence — see RealHomeOrphanWarning's doc comment
	// for why this is one of the two sidecars that needed one.
	if len(findings) == 0 {
		if warn := links.RealHomeOrphanWarning("saved findings", memDir, group+"-memory"); warn != "" {
			log.Print(warn)
		}
	}
...
// links.MemoryDir is the shared derivation this and internal/mcp's
// defaultMemoryDir both use now.
