...
// Lua dynamic-pattern catalog (issue #44 — Lua resolver slice).
	...
	swiftDynamicPatterns = []*regexp.Regexp{
	...
	// Zig dynamic-pattern catalog (issue #44 — Zig resolver slice).
	//
	...
	zigDynamicPatterns = []*regexp.Regexp{
	...
	// call sites in Flutter / Dart code:
	//
	//  1. Package-URI import ToIDs — the Dart extractor emits the full import
	//     URI as the IMPORTS edge ToID (e.g. `package:flutter/material.dart`,
	//     `dart:convert`, `src/widgets/cart.dart`). These URIs are not
	//     in-tree entity names; the resolver cannot map them to a graph entity
	//     without a package-resolution pass. Route to Dynamic (same precedent
	//     as Python relative imports and TS path-mapped imports).
	//
	//  2. Flutter/Material widget constructor calls — the Dart extractor
	//     strips the receiver from `Widget(...)` expressions and emits only
	//     the constructor name as the CALLS ToID. Flutter widgets are
	//     framework types imported from `package:flutter/material.dart` (or
	//     cupertino / widgets packages) and have no in-tree entity. The most
	//     frequent hits are enumerated below; the per-language gate keeps
	//     these names from shadowing user-defined classes in other languages.
	//
	...
	dartDynamicPatterns = []*regexp.Regexp{
		...
		// `mounted` check — bool getter on State<T>; not a call but can
		// appear as a bare reference after control-flow extraction.
		// Not applicable as a call stub; omitted.
		...
		regexp.MustCompile(`^builder$`),
		// `all` — EdgeInsets.all(), SizedBox.all(), BoxConstraints.expand()
		// pattern; most common: EdgeInsets.all().
		regexp.MustCompile(`^all$`),
