...
//
// Each per-language catalog lives in its own file
// (dynamic_patterns_<lang>.go) and registers itself via init(). This keeps
// future #44 slices from conflicting on this file — each language slice
// touches only its own file.
var (
	// crossLangDynamicPatterns are evaluated regardless of language (including
	// when the language is unknown). Template-built names (`${x}`) are
	// reflection-shaped in every language that has them.
	...
	// Each per-language file (dynamic_patterns_<lang>.go) registers its
	// entries via an init() function.
