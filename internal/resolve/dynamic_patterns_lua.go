...
// luaDynamicPatterns holds dynamic-dispatch pattern catalog for Lua (issue
// #44). The Lua extractor's luaCallTarget returns only the trailing
// identifier of a dotted call: `table.insert(t, v)` → "insert",
// `math.floor(n)` → "floor", etc. Lua global built-ins arrive as bare names
// too (ipairs, pcall, setmetatable, …). None of these names has a matching
// entity in the graph, so without these patterns they all land in
// BugExtractor.
//
// Two categories are handled:
//
//  1. Lua-unique global built-ins — identifiers that are part of the Lua
//     standard environment and are virtually never user-defined function
//     names in any other language: ipairs, pairs, pcall, xpcall, rawget,
//     rawset, rawequal, rawlen, setmetatable, getmetatable, tostring,
//     tonumber, unpack, select, coroutine.wrap → bare "wrap" (Lua-gate
//     makes this safe enough). These names land as BugExtractor on every
//     Lua corpus because the graph never contains an entity whose Name is
//     "ipairs". With the Lua language gate the risk of false-positive
//     against a user-defined `ipairs`/`pairs`/`pcall` is negligible.
//
//  2. Lua stdlib module leaf names — table.*, string.*, math.*, os.*,
//     coroutine.* leaf identifiers that are too common as user-defined
//     method names to be safe cross-language, but are safe under the Lua
//     gate because real Lua code uses them exclusively as stdlib calls.
//     Covered: insert, remove, sort, concat (table.*); format, sub,
//     gmatch, gsub, find, byte, char, rep, len (string.*); floor, ceil,
//     sqrt, abs, max, min, random, huge, pi, fmod, modf, exp, log
//     (math.*); getenv, time, clock, exit, date, execute (os.*); resume,
//     yield, status, create, wrap (coroutine.*).
//
// All patterns are gated to lang=="lua" so they cannot fire on stubs from
// Go, Python, Ruby, Java, TypeScript, or any other language (safer-bias
// rule #94). Cross-language collisions that motivated this gate: `format`
// (Python str.format), `insert` (SQL), `remove` (JS
