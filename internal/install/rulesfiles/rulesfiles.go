...
func RenderBlock(groupName string) string {
	...
	fmt.Fprintln(&b)
	fmt.Fprintln(&b, "- **Default to grafel for STRUCTURAL questions**: where is `X` defined, who calls/uses `Y`, how does a request flow end-to-end, what is the blast radius of a change, what are the modules. Reach for `grafel_find` / `grafel_inspect` / `grafel_related` / `grafel_trace` / `grafel_impact_radius` for these — **not** `grep` + reading files.")
	...
	fmt.Fprintln(&b, "| \"What does `X` look like + detail?\" | `grafel_inspect` |")
	fmt.Fprintln(&b, "| \"Who calls `X` / what does `X` call?\" | `grafel_related` (direction=callers\\|callees) |")
	fmt.Fprintln(&b, "| \"Compare reference vs candidate response/payload\" | `grafel_diff` (aspect=response_shape\\|payload) |")
	fmt.Fprintln(&b, "| \"Trace data/control flow through X\" | `grafel_trace` (kind=data\\|control) |")
	fmt.Fprintln(&b, "| \"What breaks if I change `X`?\" | `grafel_impact_radius` |")
	fmt.Fprintln(&b, "| \"Dead code / cycles / tech debt?\" | `grafel_debt` (kind=dead_code\\|cycles) |")
	...
	fmt.Fprintln(&b, "- Don't traverse imports manually — `grafel_subgraph` does it via the IMPORTS edge.")
	fmt.Fprintln(&b)
