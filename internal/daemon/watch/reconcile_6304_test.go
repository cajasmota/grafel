...
// lockAssertingBackend replaces the seams with stand-ins that fail if w.mu is
// held when the backend is called. TryLock is exact here: the only contended
// holder in these tests is the caller itself.
func lockAssertingBackend(t *testing.T, w *Watcher) {
	t.Helper()
...
// TestRemoveRepoDoesNotCallTheBackendUnderTheLock covers the teardown paths.
// RemoveRepo held w.mu across fs.Remove before #6304 as well; what #6304 added
// was the same call in the refusal unwind, which is the one the Windows job
// timed out on.
func TestRemoveRepoDoesNotCallTheBackendUnderTheLock(t *testing.T) {
	...
	// Installed after the repair so the repair's own entry watches are real and
	// RemoveRepo has more than the directory itself to drop.
	lockAssertingBackend(t, w)
	w.RemoveRepo(root)
}
