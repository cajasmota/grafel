...
// internal/extractors/yaml/helpers.go fails HERE and not only somewhere downstream.
//
...
func TestProducedEntityKindGuard7328_Yaml(t *testing.T) {
	t.Run("undeclared kind panics", func(t *testing.T) {
		defer func() {
			r := recover()
			if r == nil {
				t.Fatalf("entity accepted the undeclared kind %q: the "+
					"types.ValidateProducedEntityKind call in internal/extractors/yaml is missing or "+
		...
		_ = entity(undeclaredKind7328, "N", "sub", "q", "f.yaml", "yaml", 1, 2)
	})
	...
	t.Run("declared kind does not panic", func(t *testing.T) {
		...
		got := entity(string(types.EntityKindClass), "N", "sub", "q", "f.yaml", "yaml", 1, 2)
	...
	t.Run("recorded undeclared kind does not panic", func(t *testing.T) {
		...
		got := entity(recordedKind7328, "N", "sub", "q", "f.yaml", "yaml", 1, 2)
