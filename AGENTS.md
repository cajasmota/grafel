...
Sibling to "Evidence": that section governs whether a claim was measured, this one governs whether the *set* the claim ranges over is the whole set. The failure is a claim of totality that holds only for the subset the author happened to look at. It survives green suites, because the enumeration and the test confirming it come from the same mental model — nine instances in one week, several written by people actively hunting for the pattern. (Refs #6361)

**State what you searched FOR and WHERE, separately from what you concluded EXISTS.** Put the pattern, the tree and the tool next to the finding, so a reader can see the subset the conclusion rests on. #6365's coordinator reported "exactly four path-token FromIDs outside the tree"; measured, twelve — and five of the six missed sat in `internal/custom/`, the very tree whose omission had just been flagged.

**Derive the set from the source of truth. Where a hand list is unavoidable, make it load-bearing.** Load-bearing means a test fails in *both* directions: a listed item that no longer matches, and an unlisted item that appears. `knownInvisibleOffenders` in `internal/extractors/file_anchored_rels_guard_test.go` is the in-repo example — it cannot rot silently. Unpinned enumerations rot fast: #6349's AST proof walked `FuncDecl` bodies only and the counterexample was a package-level `var` initializer; #6357's invariant "over every emitted structural edge" filtered on a `scope:` prefix; #6345's "one chokepoint" was Pass 1 of 3; the coordinator's own heartbeat cron hardcoded an issue list, went stale, and dispatched an agent onto a closed issue.

**Name the unit of analysis for any aggregate figure.** Under-searching is not the only way in. #6375's "only 4 kinds corpus-wide have zero participation" was a *correct* enumeration over a merged 12-repo aggregate, but the property runs per group, so aggregation let kinds borrow participation across repos. Measured, ten — and the omitted set contained the kind that same PR promoted to its headline finding.

**Mark MEASURED vs INFERRED per claim, not per document, and when totality is not available name the subset rather than dropping the qualifier.** #6365 round 1 blessed six real defects through an allow-list resting on a criterion that did not hold; round 3's disclosure called `swift` a structural ref, concealing a live defect it had just certified as fine; round 4's *corrected* disclosure repeated the identical mis-attribution one item to the left, on `markdown`. A per-item "INFERRED, covers the `scope:` forms only" stops all three.
