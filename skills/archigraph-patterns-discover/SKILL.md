...
Do NOT invoke this skill on first install when no group is registered, when `archigraph status` reports indexing in progress, or as part of `/generate-docs` (that path runs its own Phase 1 + 4 already).

## Inputs the skill expects

- A registered group resolvable via `archigraph_whoami`.
- An up-to-date `<repo>/.archigraph/graph.json` per repo. If any repo's graph is stale, stop and tell the user to run `archigraph rebuild <group>` first.
- Read access to `<group>/.archigraph/patterns.json` (the pattern store) and `patterns-config.json` (thresholds — defaults applied when absent).

## Procedure

### Step 1 — Slice the codebase

Dispatch one subagent per top-level module cluster (as returned by `archigraph_list_clusters`). Each subagent receives:
...
2. If a candidate shape appears ≥ `per_subagent_threshold` times (default 2) within the slice, emit a `PatternCandidate` via `archigraph_patterns(action=record, as_candidate=true, ...)`.
...
1. Load every `PatternCandidate` (`is_candidate=true`) via `archigraph_patterns(action=query, include_candidates=true)`.
2. Cluster by trigger similarity ≥ `cluster_similarity_threshold` (default 0.8) AND at least one overlapping exemplar.
3. For each cluster with ≥ `convergence_threshold` (default 3) distinct proposer subagents, call `archigraph_patterns(action=promote, candidate_id=<id>)`.
4. Surface the promoted candidates to the user. The user's approval flips `is_candidate=false`.

Candidates that do not converge stay in the store with `is_candidate=true`. They may converge in a future run; `archigraph patterns gc` prunes them after `candidate_decay_days` (default 90) if they go stale.
...
- Never call `archigraph_patterns(action=record, as_candidate=false)` from this skill. Direct creation bypasses convergence and is reserved for the agent-task-observation path in ADR-0018.
...
- Never silently delete candidates. Stale-pruning is a separate operation under the user's control (`archigraph patterns gc`).

## Related

- `/generate-docs` — primary pattern-discovery entry point.
- `/archigraph-patterns-sync` — export the store to CLAUDE.md after discovery.
- `archigraph patterns list` — inspect what landed.
