...
Call `whoami`. Verify the resolved group matches what the user expected. If it does not, stop and ask the user to set `cwd` or pass `group` explicitly.
...
```
list_enrichment_candidates(repo_filter=["<r>"], limit=20)
```
...
If the user said "regenerate after the recent refactor", call `recent_activity(since=<timestamp>)` and tag those entities so Pass 2 prioritizes their modules.
