...
Do NOT invoke this skill if no patterns have been recorded yet (run `/generate-docs` or `/archigraph-patterns-discover` first), or if the user has manually edited the marker-wrapped block — surface the edits as candidates first via the import path.
...
- The current pattern store, loaded via `archigraph_patterns(action=query, include_candidates=false)` or via the CLI's direct file read.
...
5. Emit the marker-wrapped block via `archigraph patterns export --repo <path>` (or directly via the internal sync API).
...
1. Parse the marker block via `archigraph patterns import --repo <path>`.
2. Compare each trigger summary against the store.
3. Patterns present in CLAUDE.md but NOT in the store are surfaced to the user. The user decides whether to import them (creating new patterns via `archigraph_patterns(action=record)`) or treat them as drift to be removed from CLAUDE.md.
...
- **Keep CLAUDE.md, import to store** — call `archigraph_patterns(action=record, as_candidate=false, ...)` and re-export.
...
- `/archigraph-patterns-discover` — populate the store before exporting.
- `/generate-docs` — emits the full pattern markdown that the block links to.
- `archigraph patterns export` / `archigraph patterns import` — direct CLI surface.
