...
Write a per-finding markdown page to `~/.grafel/docs/<group>/security/findings/<fingerprint>.md`:
...
---
*Generated by grafel-security-audit Phase 2. Fingerprint: `<fingerprint>`.*
```

## Step 4 — Write index

After all findings are processed, write `~/.grafel/docs/<group>/security/index.md`:
...
---
*Run `/grafel-security-audit` again with `--since <sha>` to check only changed entities.*
```

## Step 5 — Update state

Write `~/.grafel/groups/<group>/grafel-security-audit/state.json`:
```json
{
...
Print:
> Phase 2 complete: **N** confirmed, **D** dismissed, **S** skipped. Security docs written to `~/.grafel/docs/<group>/security/`.
