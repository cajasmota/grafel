...
when-to-use: User asks to "enrich the Paths panel", "add rank and summaries to my flows", "make the dashboard Topology show data", "enrich endpoints", or invokes /archigraph-graph-enrich explicitly. Run after /archigraph-resolve (or /archigraph-graph-quality) and before /archigraph-tech-docs if you want enriched entity summaries inlined into module READMEs.
---

# archigraph-graph-enrich

Annotate the archigraph knowledge graph with structured YAML frontmatter so the dashboard's Paths, Flows, and Topology panels display rich data. This skill runs in two sequential phases:
...
- `/archigraph-graph-enrich` (slash command).

Do **not** invoke it for prose documentation (that is `/archigraph-tech-docs`). Do not invoke it to repair residual edges (that is `/archigraph-resolve`).

## Inputs the skill expects

- A running archigraph daemon (`archigraph_whoami` must succeed).
- A registered, indexed group with enrichment candidates in the queue (`archigraph_enrichments(action=list, status="pending")` returns results).
...
If `archigraph_enrichments(action=list, status="pending")` returns zero results, the skill exits with "No pending enrichment candidates. Nothing to do." (success).
...
Phase 1 always begins by fetching `archigraph_enrichments(action=list, status="enriched")` and excluding those entity IDs from all batches. This makes the skill safe to restart after any failure without re-enriching or incurring duplicate LLM costs.

State is persisted at `~/.archigraph/groups/<group>/archigraph-graph-enrich/state.json`:
...
### Pre-flight
Call `archigraph_whoami`. If it errors: abort with "archigraph MCP not configured for this directory. Run `/mcp` to fix."
...
3. Build entity stubs with `archigraph_expand(node=<id>, depth=2)` for neighbour context.
4. Partition by `criticality_band` → Sonnet vs Haiku batches.
5. Print cost estimate; gate on user confirmation (or `--yes`).
6. Dispatch batches, write YAML frontmatter to doc files, call `archigraph_enrichments(action=submit)`.
...
- `archigraph_enrichments` — primary tool (`action=list|submit|reject`).
- `archigraph_expand` — depth-2 neighbour context for entity stubs.
- `archigraph_save_finding` — persist the Phase 2 validation report.

## Output layout

```
~/.archigraph/docs/<group>/<repo-slug>/enrichments/
  http_endpoint/
...
Frontmatter is prepended to existing doc files when a doc already exists; a minimal file is created otherwise. The canonical placement for new files is `~/.archigraph/docs/<group>/<repo-slug>/enrichments/<kind>/<entity_id>.md`.
...
- ADR-0015 — residual repair; run `/archigraph-resolve` before enrichment to minimize "orphan endpoint" findings.
- `skills/archigraph-resolve/SKILL.md` — upstream skill for residual resolution.
...
→ `/archigraph-security-audit` — run a two-phase security audit against the enriched graph.
