...
- **HCL `CONTAINS`/`DEPENDS_ON` resolve at nested paths (#6367).** Measured on one fixture at two paths: at `infra/envs/prod/main.tf` all 5 `CONTAINS` and both `DEPENDS_ON` dangled; at the repo root the two kinds diverged — the 5 `CONTAINS` resolved onto the file component by accident, while both `DEPENDS_ON` edges were **misanchored** onto that same component, having left the resource that declares them. `DEPENDS_ON` was wrong in both columns. The split is the file component's name: it is the basename, so a path-valued source matches only when the path is its own basename.
...
- **This release requires a reindex of existing graphs.** After upgrading, force a rebuild per group, as [docs/quickstart.md](docs/quickstart.md) describes:
  ...
  - **The files were parsed, but the extractors now produce different output.** A rebuild is what re-runs them. This is why Solidity (#6423, entity recall 19/40 → 40/40 on the graded fixture; #6368), F# (#6326, #6336), Groovy (#6370), Svelte (#6366), Verilog and Astro (#6298), HCL (#6367), Razor and Vue (#6372), Java/Spring (#6429), TypeScript/Angular (#6433) and Python/Django and FastAPI (#6417, #6385) all yield materially different graphs after upgrading. Two further fixes reach beyond any one language. A real declaration now outranks an import placeholder in the by-name index (#6369), which is language-agnostic because the collision is by name — so graphs can change after a rebuild in repos that touch none of the languages named above. And extraction skips `ERROR` subtrees (#6360), which applies to any tree-sitter language but only to files that actually contain a parse error; measured on 28 canary fixtures, only Groovy's output changed.
