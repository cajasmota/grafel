...
- **Machine-generated source is detected, stamped and ranked below authored code (#6329).** A file whose first contiguous comment block carries a generator banner is recognized, every entity from it is stamped `generated`/`generated_by`, and those entities are demoted in search results by rank tier rather than by a score penalty — a penalty would vanish wherever an embeddings sidecar exists, because rank fusion replaces score magnitudes with rank reciprocals. Additive and reversible: no entity counts change and no content is dropped. Detection scans only the leading comment block, with a tolerance for a banner sitting behind a leading declaration (#6348); a 20-line window was measured and rejected, because it matches hand-written prose — generator scripts carrying the banner they emit, docs quoting the marker, test drivers naming a data file. MEASURED on 11,740 module-cache candidates, 8,544 → 8,506: 38 removed, 0 added. The ranking *benefit* is explicitly unverified: both measurement repos are ~0.1% generated by line against the requester's 47.6%, so the mechanism is proven locally and the payoff cannot be.
...
- **Inheritance from .NET Framework base types resolved in this same release (#6337) — read the numbers with the metric named.** In the reporter's codebase 36 of 37 `Inherits` clauses point at a framework base (`System.Web.UI.Page`, `System.Windows.Forms.Form`), and the extractor-side figures above (`EXTENDS` 153/289) are the in-tree bind count. Measured on the same three trees with the disposition classifier the feedback report uses, `EXTENDS` goes **191/289 → 288/289** and `IMPLEMENTS` **97/205 → 189/205**, i.e. hierarchy resolution **58.3% → 96.6%**. The residual 17 is stated rather than rounded away: 16 are `IFrameServer.*` member-level clauses whose interface *is* declared in-tree — a resolver-routing defect that stays dangling and stays reported — and 1 is `BooleanConverter`, a real BCL type not yet on the curated allowlist. Two limits remain by design: a bare `Form` and a dotted `System.Windows.Forms.Form` produce different external nodes, because recovering the namespace from a bare leaf would be guesswork; and the arm is gated to VB.NET, so C#, F#, Java and Python still count a framework base as an extractor bug and want their own measured allowlists.
...
- **Overload identity is fixed for declarations, not for call targets (#6440).** Two overloads of a method in one type used to share an entity id and merge into one node; that is fixed in this release (see the Fixed section below), re-measured at 279 colliding groups rather than the 4 this limit previously cited. What remains: a `CALLS` target is still rendered as a bare name with no arity, so a call to an overloaded method binds to whichever overload keeps the bare name — the first in source order — rather than to the one its argument list selects. That choice is deterministic from file content and matches the previous survivor, so no id moves today, but inserting a new overload *above* the current first would move the bare name onto it.
...
- **Angular `HttpClient` call sites are extracted, so the frontend half of a cross-stack link exists (#6433)** — thanks @auxmedrano. The reporter measured 98 `SCOPE.ExternalAPI` entities across a monorepo, all backend and zero frontend, against 42 frontend files making concrete `this.http.get/post/patch(...)` calls. Four independent guards each rejected idiomatic Angular, and each was individually fatal — fixing any one alone still yielded zero nodes. The JS/TS patterns that mint `SCOPE.ExternalAPI` anchored on the bare identifiers `fetch`/`axios`, which a receiver-style `this.http.get<T>(…)` matches under none of. The Nest synthesizer early-returned before its regex ever ran unless the file text contained the literal token `httpService`/`HttpService`, which `private http = inject(HttpClient)` contains neither of. The URL-shape requirement admitted only a quoted literal starting `/` or `http`, or a template literal containing `${…}`, so the reporter's `base(code)` and `` `${BASE}/${id}` `` both failed it independently of the token guard. And the REST early-exit guard rejected the file before the Nest synthesizer was reached at all, an Angular service containing no `fetch(`, no `axios.` and no `$`. All four now admit receiver-style HTTP clients, with dynamic URLs emitted as a call site rather than dropped.
...
- **Django REST Framework routes point at the ViewSet instead of at their own path (#6471).** The synthesizer passed the route's *path* as its own handler reference, stamping `source_handler = "Route:<own path>"`. Unlike the identical shape in Spring (#6429), this one did not dangle — it resolved, because `Route` entities are indexed into the phase-2 handler map, so the same-file lookup found the very entity the synthesizer was iterating and a consumer asking "which handler serves this route" got the route back. A wrong edge that resolves is worse for an agent reading the graph than a visibly absent one, and the total edge count is unchanged, which is why no count-based metric could see it. Routes composed from `router.register(prefix, ViewSet)` now carry `handler_class` and bind to the `View` entity; the attribute form (`router.register(..., views.UserViewSet)`) deliberately leaves it unset rather than inventing a name, because a fabricated handler resolves to nothing and, on the corpus path, deletes the endpoint outright.
- **Django: two URLconf roots mounting the same prefix both survive (#6417).** The nested-URLconf dedup keyed `url_mount_point` synthetics against a map whose lifetime spanned every parent file in the pass, so when `app/urls.py` and `admin/urls.py` both declared `path("api/", include(…))` the second was dropped outright and the survivor carried whichever file was scanned first — reversing the scan order flipped the attribution. The map is now per-file, matching what the FastAPI emitter already did; the child-route ids stay shared, because those genuinely are one composed route however many parents reach them.
- **Angular class methods are recognized as the caller of the HTTP call they make (#6447).** The JS/TS enclosing-function index matched three declaration shapes and explicitly declined the fourth — object/class method shorthand — so an HTTP call made from a class method had no caller to attribute to. Measured over the `angular-http-mini` fixture rather than inferred: **5 of 7** `HttpClient` call sites had an empty caller and **2 carried the wrong one**. The empty case is silent by construction — no `source_caller` means the resolve step never runs and even its broad same-file fallback never fires, so the result is zero `FETCHES` rather than a coarse one. The wrong case is worse: spans carry a start offset and no end bound, so the nearest-preceding walk never left a top-level arrow-function `const` declared above the class and stamped it on call sites inside it. The fixture goes to 17/17 relationships with 0 forbidden hits; before the fix it had contained no `FETCHES` row at all, so the defect was unobservable. The root cause is explicitly **not** fixed: spans still have no end bound, and the mis-attributed site now binds correctly only because the method header sits nearer to the call than the helper does — filed as #6500. Three non-`FETCHES` consumers of the same index also move, all in the improving direction: two SSE handlers in one class stop collapsing onto a single file-named node (an entity-id move), and WebSocket and GraphQL-subscription edges retarget from dangling `Function:<file>` placeholders onto the real operation.
- **A route can no longer be its own handler, in any framework (#6485).** `Route` entities were eligible handler targets in the phase-2 handler index, so a failed handler ref could bind to a `Route` named after the same path and emit `Route:/foo --IMPLEMENTS--> /foo`. The Spring (#6429) and DRF (#6471) fixes above each removed one producer; this removes the class, by gating index construction rather than the drop site. Two paths had to close, not one — the same-file exact lookup fires *before* the existing drop, and the same-file bare-name index is keyed without a kind at all, so a `Controller:` ref bound to a same-named `Route` whatever the drop said. An endpoint whose only handler ref was such a placeholder is now **kept** on both the CLI and daemon paths instead of dropped: the evidence says the ref is a placeholder, not that the route is absent, and dropping it would have made the two paths disagree about a real endpoint's existence. A genuine handler competing with a same-path route still wins, and the orphaned-bare-name case keeps its existing #6150 verdict rather than being widened. The before/after distribution comes from a constructed harness, not a corpus index: every fixture producer of this shape had already been repaired, so a corpus run would measure 0→0 and prove nothing.
- **A `fetch` built from an imported base-URL constant matches a handler in the same repo (#6450, Task 1 of 2).** The substrate constant fold ran only in `grafel group-link`, while the per-repo call→definition matcher runs at index time and joins on the `path` property — so the two passes structurally never met and `` fetch(`${BASE}/things`) `` was written to `graph.json` as `UNRESOLVED_FETCH` even when the handler sat in the same tree. No gate was involved. The fold now runs at index time, before handler resolution. Only `path` is rewritten; `Name` and the derived entity id stay frozen at the unfolded form, because folding at extraction time would churn the id of every dynamic-base-URL call site on every incremental run. Measured by diffing every `(kind, name, source_file, id)` tuple with and without the fold: **0 lost, 0 changed**, with 3 new cross-stack process flows that could not previously exist. The defect reproduces only when the constant is imported from another file — a same-file constant is already folded by the JS canonicaliser at extraction time. Two limits are stated rather than fixed: a constants module that produces no entities is invisible to the symbol table, and the incremental one-file path does not fold, because a one-file slice cannot see the declaring module. This is Task 1 of 2 — the consumer-side resolver arm is untouched and **#6450 stays open**.
- **Three FastAPI rules anchored on a name that file never mints (#6444).** `fastapi.yaml` hung `IMPORTS`, `DECORATES` and `INJECTED_INTO` on `Route:<function>`, but Route entities in that file are minted only from the decorator path, the `APIRouter` variable or the filename — never a handler function name. Every decorated handler therefore emitted three permanently-dangling edges, probed end to end with the real detector before any change. The three rules now anchor on the `SCOPE.Operation` the generic Python extractor actually mints, and the edge count is unchanged at 57 → 57. There was no FastAPI golden fixture at all; `python-fastapi-mini` is new and graded 25.0% relationship recall before the change. The sibling sweep across flask, django, axum, vapor, strawberry and symfony is **not** included — one of those (`actix_web.yaml`) is a false positive whose "fix" would be a regression, and two may have no YAML-layer fix. The class is also invisible to the resolver-bug metric: yaml-driven edges carry no language property, so these stubs are classified as dynamic and excluded from the rate.
- **Process-flow cross-stack verdicts are decided by gates that can fire (#6458).** Three kind literals in `process_flow.go` went stale at the #1217 endpoint-kind split, and two of them consequently returned false unconditionally — the same silent-disable shape as #6449. Repairing them revived a defect the dead gate had been hiding: the file-coarse consumer fallback predates #1639, whose correction was encoded only inside `chainCrossesRepoBoundary`, because the inert gate left nothing to reconcile at the time. A uniform three-line repair would have shipped that silently. Consumer endpoints are now resolved **per endpoint** rather than chain-globally, which is what the probes required: `hasContinuation` is computed once for a whole chain, so a chain that resolved one HTTP call in-repo and then made a second, unresolved one was reported intra-repo with an empty reason. Measured on a real indexed group graph (427k entities, 1.85M relationships, 282 emitted flows, 114 consumer synthetics): the false-negative class this closes is **empty there** — 0 chains of either shape, `cross_stack` true for 0 of 282 — so the fix rests on the probes and on correctness, not on a corpus delta. Two documentation claims the code had never implemented are corrected alongside, in two different files: the `crosses_external_lib` header in `process_flow.go`, and a `types/kinds.go` note asserting that no extractor emits the pre-split kind — there are in fact nine live producer-side emitters of it. A third stale literal is a code fix rather than a comment: the response-shape handler table still mapped `Route` to the single pre-#1217 kind, and is widened to all three, pinned by the first test that lookup has ever had.
...
- **protobuf: a service sharing a name with a message keeps its `CONTAINS` edge (#6459, partial).** A `service Foo` declared alongside a `message Foo` in one file lost the file→service edge: the service's structural ref was claimed by the sibling and then dangled. The fix is an **ordered tier** consulted only after the operation-family lookup misses and only when that family holds nothing at the same (file, name) — not a widening of any kind family, which two earlier attempts tried and which was measured as a corpus-wide regression in both cases. Family widening is lossy in a way that is easy to argue backwards: the family lookups return "no match" when two members collide, so *adding* a member turns unique into ambiguous and destroys bindings that previously worked — measured on `python-dramatiq-mini` as 11 rewrites/0 ambiguous → 9/2, losing two Celery `CALLS` edges, and in the proto-only variant as one correct `service Admin → rpc User` edge lost for zero gained. Every kind family is byte-identical to the previous release. The residual is stated and pinned rather than claimed closed: `service Foo` containing `rpc Foo` still leaves the service with zero inbound `CONTAINS` and the rpc with two, because both refs render to the identical string — closing that is a ref-*format* change on its own issue. A related finding, that the proto extractor uses an unvalidated import string literal verbatim as an entity name, is filed as #6497.
...
- **One import can no longer unresolve every edge to that name repo-wide (#6369).** The resolver indexed the per-import placeholder record — minted by a dozen extractors (css/scss/less, graphql, vue, kotlin, razor, proto, markdown, cpp, just, fish, javascript, cross/imports) — into its by-name table exactly like a real declaration. Adding one file whose import's last segment collided with a base type name flipped that name ambiguous and dropped every bare-name `EXTENDS` to it across the repo, including edges in files that imported nothing. A real declaration now outranks a placeholder. The reach is language-agnostic rather than universal: it changes output only where an import placeholder exists, but because the collision is by name rather than by language, graphs can change after a rebuild in repos that touch none of the languages named elsewhere in this release.
- **A function-local `const` no longer captures an imported package name repo-wide (#6467)** — thanks @arthurgeron. The by-name precedence introduced by #6369 ranked a record on one property, whether it is a per-import placeholder, so every non-placeholder counted as a declaration of the name and could take the repository-wide slot without either arm looking at where it was declared. A `const` emitted inside a function body is not a placeholder, so it won the slot for the whole repository and an import of a same-named package resolved to it instead of staying unresolved for the external-library binder — `import { useToolkit } from "toolkit"` retargeted from `ext:toolkit` onto an unrelated local `const toolkit` in a file that imports nothing. A record that is not addressable outside the callable that declares it can no longer take the slot from an import placeholder nor keep it against one; the pairing collides into ambiguity, so the bare-name lookup and the external synthesizer run as before. This is a regression shipped in #6369 earlier in this same cycle. Both locality signals are written only on the JavaScript/TypeScript path, so no other language's output changes.
- **VB.NET inheritance from framework base types no longer counts as an extractor bug (#6337).** Every arm of the external synthesizer was gated on an `IMPORTS` or `CALLS` relationship or a structural-ref stub, so an `EXTENDS`/`IMPLEMENTS` target had exactly one path open to it — a generic dotted-root fallback that left a bare `Form` as a raw stub, collapsed every `System.`-rooted BCL type onto a single `ext:System` node, and, measured live, bound `Windows.Forms.TextBox` to `ext:Windows` on the authority of the Rust `windows` crate. Hierarchy resolution on the 302-file VB corpus goes **58.3% → 96.6%**; see the VB.NET section above for the residual and the guards that keep a typo or a misparsed clause a visible defect rather than a fabricated external node. Deliberately gated to VB.NET — the issue's title says "in every language" and the other languages are not addressed here.
- **VB.NET member calls bind through the receiver's declared type (#6454).** A qualified member call previously emitted no `CALLS` edge at all, and the one path that did emit rendered its target from the receiver's spelling rather than its type. Both halves are fixed: `CALLS` goes from **5,059 to 8,044** total and **3,386 to 4,166** resolved. The resolution *rate* falls (67% → 52%) because most of the 2,985 newly-emitted sites name a .NET Framework type with no source in the tree; see the VB.NET section above for the full disposition, the precision sample and the scope limits.
- **VB.NET overloads no longer share one entity id (#6440).** Two overloads of a method in one type hashed to the same `EntityID`, so the dedup kept the first, dropped the rest and **unioned the dropped records' edges onto the survivor**, with no counter and no log — a `Dispose()`/`Dispose(Boolean)` pair minted a phantom self-`CALLS` on a method that does not call itself. Re-measured with the real extractor on the same 302-file corpus, the figure this was deprioritised on was **~70× low: 279 colliding id groups and 364 dropped records of 13,479, not 4** (the old number was scoped to the `Implements`-carrying subset it happened to be measured for). After: 5 groups, and those 5 are not overloads but two files each declaring a type name twice, which the bare-name rule for types accepts by design. Only members that actually collide get a discriminator — the declared parameter-type list, with parameter names excluded so a rename cannot move an id — and the first overload in source order keeps the bare name, so no *declaration* id moves and bare-name call targets still land: at-risk `CALLS` are 566 resolved / 707 dangling before and after, identical. Three derived `SCOPE.Process` flow node names do move, and 2 new flows appear; flow nodes are re-derived on every index, so there is no carried-forward-state consequence. An incremental re-index across the fix was run on the production path: editing a colliding file yields a graph identical to a clean full index with 0 id mismatches; editing an unrelated file leaves the colliding file un-split until it is next touched, with no *additional* dangling edges.
- **F# never received the import-placeholder fix, so one `open` could still delete a type's inheritance topology repo-wide (#6369, F# arm).** The precedence rule added earlier in this cycle keys on an import placeholder's subtype marker, and the F# extractor emitted its per-`open` placeholder with no subtype at all — so the predicate never saw it and the original defect stayed live for F#, the language whose reproduction was the strongest evidence on the ticket. The marker is now stamped at the extractor, and the placeholder carries the full module path as its specifier, the way Razor and Vue already do.
...
- **Groovy graphs have inheritance topology (#6370, partial).** Groovy was in neither of the two paths by which a language gets `EXTENDS`/`IMPLEMENTS`. Measured on the checked-in fixtures alone, 3 `EXTENDS` and 2 `IMPLEMENTS` were being dropped; the edges are now emitted from the Groovy extractor itself, so no duplicate component per type is minted. This is the Groovy arm alone — the audit on the issue names eight further languages in the same position, none of which are addressed here, and #6370 stays open.
...
- **HCL `CONTAINS`/`DEPENDS_ON` resolve at nested paths (#6367, partial).** Measured on one fixture at two paths: at `infra/envs/prod/main.tf` all 5 `CONTAINS` and both `DEPENDS_ON` dangled; at the repo root the two kinds diverged — the 5 `CONTAINS` resolved onto the file component by accident, while both `DEPENDS_ON` edges were **misanchored** onto that same component, having left the resource that declares them. `DEPENDS_ON` was wrong in both columns. The split is the file component's name: it is the basename, so a path-valued source matches only when the path is its own basename. This is the HCL arm alone: the issue covers seven sites across six extractors, and graphql, proto, bicep, dockerfile and swift are unchanged, so it stays open.
...
- **Files with a syntax error no longer emit entities from the broken region (#6360).** A whole-file error ratio gets the extremes right and the middle backwards: a file with one malformed declaration parses at 0.0033, sails under the gate and is handed to the extractor in full, `ERROR` subtree included — so whatever is built out of that region is a reading of tree-sitter's error recovery, not of the source. `ERROR` subtrees are now hidden from the node accessors, making the emitted set a true subset of what the file declares. This can change output for any tree-sitter language, but only for files that actually contain a parse error: measured over the 28 in-tree canary fixtures, 27 show a reachable-node delta of exactly 0 and only Groovy changed, by −6. The silent-incompleteness half — a partially-parsed file still reporting as healthy — remains open on #6360.
...
- **The four ASP.NET Web Forms siblings of `.aspx` are named in the skip report (#6448).** The unsupported-extension table listed `.aspx` but not `.ascx`, `.asmx`, `.ashx` or `.master`, none of which appeared anywhere in the classifier package — so a Web Forms repo saw its user controls, web services, handlers and master pages fall through to a generic `unsupported_extension` skip with no language name and no per-extension tally, while `.aspx` in the same tree got the named, actionable row. Four table rows and their assertions; the extraction of those file types is unchanged and still absent. The issue's open question — whether a generic derive-don't-list check would have caught this — is answered no: that proposal (#6382) was closed on a measurement showing it would fire on nothing, so the table stays hand-maintained.
...
- **`grafel_related direction=uses` traverses every relationship it names (#6314, partial)** — thanks @manuel1358000. The route was pinned to a single edge kind, `NAVIGATES_TO`, so every `CALLS`, `USES`, `USES_HOOK` and `REFERENCES` edge in the model was unreachable from a parameter named `uses` — and the response said nothing about it, returning a confidently empty answer. All five kinds are now traversed in both directions. This is one arm of #6314: the report's other two findings — `grafel_find` (bm25) ranking unrelated-domain matches above the relevant file, and `direction=uses` missing property and enum-member access — are not addressed, and the issue stays open.

#### Indexing, daemon, and platform

- **A named pipe with an indexed extension no longer hangs an indexing worker forever (#6416).** The walker branched only on `IsDir()`, so a FIFO named `Hang.vb` was returned as a candidate and opened — and `open(2)` on a FIFO does not fail, it waits for a writer, with no timeout, no error and no log line. Any unprivileged user could stall indexing with one `mkfifo` inside a watched directory; a character device such as `/dev/zero` is the same bug in its second shape. The walker now requires a regular file, covering pipes, devices, sockets and anything a future platform reports as irregular, and it *reports* the skip rather than dropping it silently — a source-looking file that produced no entities and was reported nowhere is the diagnosis cost #6338 exists to remove. Symlinks are decided on the target, so a link to a real file is still indexed. The incremental path delegates to the same walker and needed no separate fix.
...
- **Repair candidates are surfaced again (#6449).** The dynamic-base-URL collector gated on the kind literal `http_endpoint`, which the #1217 split replaced with `http_endpoint_call` for consumer-side synthetics — so on **every graph indexed since that split** it returned zero candidates. It runs unconditionally on both the inline and background-trickle indexing paths, so `grafel_repairs action=list` and the dashboard curation handlers were silently empty. Its test fixture hardcoded the dead kind, which is why nothing failed. The write-back path (`--enable-repair-apply`) is default-off and unaffected either way: candidates are surfaced again, the end-to-end apply flow still requires its own flag.
- **A secret scan that skipped a file says so, instead of reporting the repo clean (#6483).** `ScanPath` reported skipped files only to stderr — which, for the two daemon call sites, *is* the daemon log the client never reads — so an MCP or dashboard caller received an unqualified "clean". Two of the four skip reasons were worse than under-reported: over `max_size` and "line longer than 64 KB" were recorded **nowhere at all**. Both are reachable rather than theoretical: the dashboard exposes `max_size` as a query parameter, so a caller passing `max_size=1024` got a clean verdict for a repo in which nearly every file was skipped; and the 64 KB line limit sits under a 512 KB size cap with no denylist for `.json`, `.lock`, `.map` or minified `.js` — the files that carry long lines. The long-line case was also **discarding real findings**: a probe with a genuine AWS key on line 1 and one long line below it reported `findings=0 skipped=[]`, versus `findings=1` for the identical key without the long line. Partial findings are now kept for that reason alone, with line numbers asserted against a real scan for the first time, and the four reasons are distinct client-facing strings. The API changed shape deliberately — `ScanPath` returns a struct rather than a third return value, so no call site can `_`-discard the skips, which is the exact failure being fixed. The credential miss itself is tracked separately as #6505, and a size-gate `lstat` hole as #6507. Intended filters (extension denylist, test files) stay unreported.
- **The two incremental manifest saves stamp the commit the pass read, not the commit at save time (#6474, partial).** `SaveManifest` shelled out to `git rev-parse` at save time, so a commit landing during a pass labelled the graph with a commit it was not built from. The pass-start capture is now threaded into both incremental saves, and hoisted to immediately before the source walk — it previously ran after the walk, which was wrong on its own terms. The consequence is stated narrowly: there is exactly one *functional* consumer, the head-advance detector, and an over-advanced label there is self-concealing — the next pass sees `HEAD == manifest`, finds zero changes and reports done over a graph built from an earlier commit. The rest are reporting surfaces (statusline, dashboard, RPC status and `grafel_index_status`, which reports `AtHead: true` over a stale graph). No automatic reindex is keyed on that flag, so this is one detector plus three displays, not staleness detection generally. The non-incremental CLI path at `cmd/grafel/index.go` is **deliberately untouched** and filed as #6502: its `graph.fb` header carries the identical defect, and fixing only its manifest would make the two disagree about the same graph.
...
- **The feedback report's orphan gate can fail below 100% (#6346).** The sanity check passed a kind unless *every* entity of that kind was orphaned, so one resolved edge anywhere — including an accidental one — flipped the kind to passing for good. `http_endpoint_definition` at a 99.6% orphan rate (#6313) was therefore reported as PASS, with the confidence score unaffected and nothing in the report saying anything was wrong; the reporter found it by reading the orphan table himself. The metric also counted only outgoing edges, so kinds that are legitimately sinks were miscounted as orphans.
- **A document emitted twice no longer silences the orphan-rate gate (#6378).** The gate divided unique orphan entities by a per-document occurrence total, so duplicating a document doubled the denominator and left the numerator alone — a functionally unwired kind measured at 91.7% orphaned dropped to 45.8% and the check went quiet, which is the direction that costs the most.
- **Kinds that are terminal by design stop failing the participation check on every run (#6377).** `kind-carries-semantic-edges` fired on kinds that correctly have no outgoing semantic edges — markdown code fences, CSS selectors, `requirements.txt` entries — measured per group across 12 corpus repos at 14 firings over 10 distinct kinds, roughly 14% of them true positives. The gate now defers to a kind's history when that kind has any participation on record, so the report's failures mean something again.
- **14 language records no longer claim the fixture-proven coverage tier without a fixture (#6432).** The `✅ full` tier is published to users, in the legend on every by-language page, as "fixture-proven, resolves the general case" — but no code path related a status to a measurement, so `full` had only ever meant that someone asserted it. Of the 22 records claiming it, 14 had no graded fixture, no baseline entry and no recall figure anywhere in the tree (assembly gnu-as/masm/nasm/riscv, Clojure, COBOL core and both runtimes, Crystal, Dart, F#, JCL, Lua, Nim); three of those asserted ✅ with an empty notes field. They are retagged `partial`, which is the honest tier the vocabulary offers, and each carries the rule that would lift it back: a graded fixture whose recall is recorded in `baseline.json`. The backed records now cite their fixture and their number, so backed and unbacked stop rendering alike.
- **A single registry language tag can no longer invent a coverage-matrix row (#6353).** The matrix's language rows are the union of two sources — slugs derived from the extractor directories, and `language` tags on `docs/coverage/registry.json` records. #6351 bound the first to the real tree and left the second entirely unguarded, and the defect was live in shipped docs: one record tagged `javascript` against 84 tagged `jsts` rendered a spurious `[javascript]` row in `summary.md` directly below the real `[JS/TS]` one, and a whole `by-language/javascript.md` page behind it. Registry tags must now be derived slugs or justified exceptions, checked in both directions so a dead exception rots visibly. Deriving that exception set rather than accepting the proposed candidate list mattered: **four of the five candidates were already derived slugs**, so pasting the list would have shipped four dead entries on day one. Exactly one genuine exception remains, `multi` (126 cross-cutting Docker/Terraform/Actions records), which renders no row and would be a lie to retag. Docs delta: the spurious page is deleted, JS/TS "Other" goes 9 → 10, and the headline count 40 → 39. This had to be an inline gate — the generic meta-check proposed for this class (#6382) was closed on a measurement showing it would fire on nothing.
...
Nothing here changes a graph. It is listed because most of the defects above were found by measurement rather than by a failing test, and this is the machinery that did the finding.

- **The first graded Solidity fixture (#6424)** — 52.8% entity recall on arrival, with all seven suspected defects confirmed by measurement rather than by inspection; #6423 and #6368 came out of it. **A graded protobuf fixture (#6453)** pins the resolver-visible half of #6359, #6422 and PR #6419; the #6422 half is not gradeable through the current fixture vocabulary and is stated as such rather than faked with an assertion that cannot fail.
- **The fixture grader stopped diagnosing its own bugs as extractor misses (#6476).** Every `to_bare_name` row was reported as resolved, so a row that could not match was attributed to the extractor. Related, and fixed alongside it: **a fixture row naming a file is answered only by that file (#6464).** 47 fixture `source_file` paths resolved to nothing and 4 were bound to the wrong entity; those rows are corrected, and the fallback behind them — a file-narrowed lookup silently retrying unnarrowed, so a same-named entity in a *different* file could answer the row — is closed at all three call sites: when a row specifies a file, the file-narrowed lookup is the only answer accepted. Pinned by `TestKindBlankScanDoesNotRescueAWrongFile`.
- **A golden fixture row can assert an entity's subtype (#6488, arm A only).** `ExpectedEntity` had no subtype field and `Subtype` is not part of the entity id, so the field most implicated in this cycle's defects — an enum value relabelled a field, a message reparented onto an rpc — was invisible to both identity and the grader. A row can now pin an exact subtype, including discriminating two candidates that share the whole `(kind, name, source_file)` key, and blank is a value rather than a wildcard. All 26 existing fixtures were graded before and after: **zero differences**, so an absent `subtype` still means "don't care". The arm's own scope correction is recorded rather than glossed: the import-placeholder case #6488 cites as its example is *not* assertable at any fixture, because placeholder pruning removes every such entity before the graph is written, so #6481 stays observable only through extractor-level tests. Relationship endpoints are untouched, and the other arms of #6488 (`forbidden_entities`, `from_bare_name`, totals) are separately blocked, so the issue stays open.
- **The rename-parity baseline was built into an empty state dir, so one side of the comparison was a no-op (#6482).** Test-only — the indexer is untouched, and both halves of the issue's title were wrong. Nothing is invented: every `RENAMED_FROM` edge is unbound in its own graph *by contract*, so "fixing the dangling target" would have deleted a shipped feature while turning the parity test green. And the delta was manufactured by the harness: the reference side built its baseline into a fresh temp directory, so rename detection had no previous graph to load and could not emit the edge, while the incremental side ran over an already-populated one. `RENAMED_FROM` is now ignored by the parity comparison **and** paired with a positive assertion that the incremental run emits exactly the one expected edge — the ignore entry could not land alone, because a mutant widening that same ignore map by one edge kind was measured to turn a *failing* parity test green. Two previously-skipped parity tests now run unflagged, and a gate that hardcoded #6461's cause for tests that do not exhibit it now takes the cause as a parameter.
- **The daemon's stale composed endpoint is pinned by a test, not fixed (#6461).** A Django route rename on an unchanged mount file loses one entity and invents another; the test records the behaviour so the eventual fix has a baseline. The issue remains open.
- **Gates that could not fire, and tests that could not fail.** The coverage-matrix gate could not fire on the change that makes the matrix stale (#6332); the citation gate sat behind a path filter that 76.8% of cited files could not trip (#6352); a shutdown-boundedness test asserted a coin flip (#6373, partial — the flaky-PASS shape is fixed; the deadline that flaky-*fails* under load is still on the open ticket); and a Strawberry fixture implied `include_router(prefix=)` coverage that nothing read (#6387).
- **Measurement and contributor docs.** An incremental phase-trace harness with its coverage claim proved from the AST (#6199); an Evidence section in `AGENTS.md` requiring real-tree measurement and no unverified claims (#6347); and the supported-language set derived rather than hand-listed (#6361).
...
- **This release requires a reindex of existing graphs.** After upgrading, force a rebuild per group, as [docs/quickstart.md](docs/quickstart.md) describes:
  ...
  - **The files were parsed, but the extractors now produce different output.** A rebuild is what re-runs them. This is why Solidity (#6423, entity recall 19/40 → 40/40 on the graded fixture; #6368), F# (#6326, #6336, #6369), Groovy (#6370), Svelte (#6366), Verilog and Astro (#6298), HCL (#6367), Razor and Vue (#6372), Java/Spring (#6429), TypeScript/Angular (#6433, #6447, #6450) and Python/Django and FastAPI (#6417, #6385, #6471, #6444) all yield materially different graphs after upgrading. VB.NET hierarchy edges change again on top of the extractor itself, because external synthesis for `EXTENDS`/`IMPLEMENTS` (#6337) runs at resolve time. Two further fixes reach beyond any one language. A real declaration now outranks an import placeholder in the by-name index (#6369, with the locality regression it shipped fixed in #6467 and its F# arm in the same issue), which is language-agnostic because the collision is by name — so graphs can change after a rebuild in repos that touch none of the languages named above. And extraction skips `ERROR` subtrees (#6360), which applies to any tree-sitter language but only to files that actually contain a parse error; measured on 28 canary fixtures, only Groovy's output changed. And `Route` entities are no longer eligible handler targets (#6485), which is framework-agnostic: it deletes self-referential `IMPLEMENTS` edges and changes endpoint retention on the CLI path, so a rebuilt graph can move in repos using express, flask, axum or symfony — frameworks named nowhere else in this note.

  - **Some VB.NET entity ids move, and the scope is narrow enough to state exactly (#6440).** Only members that *collide* — two or more overloads of one name in one type — are given a discriminator and therefore a new id. Every non-colliding member keeps its id byte-for-byte. Measured by diffing every `(kind, name, source_file) → id` tuple across the full 302-file corpus: 13,234 → 13,595 entities, 364 keys added, 3 removed, and **0 shared keys whose id changed**. The 3 removals are the derived flow nodes described below. On the incremental path the same probe reports 0 id mismatches against a clean full index. The first overload in source order also keeps its bare name and its existing id, so bare-name `CALLS` targets still land — measured identical before and after at 566 resolved / 707 dangling. Derived `SCOPE.Process` flow node names embedding a split member do change, but flow nodes are re-derived on every index and are stripped from carried-forward state, so nothing stale survives a pass. On an incremental run, a colliding file that has not been touched simply stays un-split until it is — the same self-healing staleness as any other extractor improvement, not a half-rekeyed graph.
