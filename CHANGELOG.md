...
- **Angular `HttpClient` call sites are extracted, so the frontend half of a cross-stack link exists (#6433)** — thanks @auxmedrano. The reporter measured 98 `SCOPE.ExternalAPI` entities across a monorepo, all backend and zero frontend, against 42 frontend files making concrete `this.http.get/post/patch(...)` calls. Three independent guards each rejected idiomatic Angular, all confirmed by test before the change: the JS/TS client patterns that mint `SCOPE.ExternalAPI` anchored on the bare identifiers `fetch`/`axios`, so a receiver-style `this.http.get<T>(…)` matched none of them; the Nest synthesizer early-returned unless the file text contained the literal token `httpService`/`HttpService`, which `private http = inject(HttpClient)` does not; and the REST early-exit guard rejected any file containing no `fetch(`, no `axios.` and no `$`. All three now admit receiver-style HTTP clients.
