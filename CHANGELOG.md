...
Requested in #6321 by @dcastro-imp: ~670 `.vb` files in a legacy WinForms/.NET Framework codebase produced zero entities, because `.vb` was dropped at classification before any extractor was reached — nothing errored and `doctor` reported the group healthy. All measurements below are from one 302-file corpus of three open-source WinForms projects (WakeOnLAN, StaxRip, display-drivers-uninstaller; 13,479 records), run through the same resolution chain the CLI runs. It is one sample on three desktop projects, not a universal recall figure.

- **Unsupported files are reported instead of silently dropped (#6338):** `doctor` and `status` now carry a per-extension "N files skipped, language not supported" row. Previously the only way to notice was to cross-check `find -iname "*.vb" | wc -l` against the entity count by hand. This is language-agnostic — it closes the same gap for every extension no extractor claims.
- **`.vb`, `.vbproj`, `.bas` and `.cls` classify as `vbnet` (#6355)**, and reach a hand-written recursive-descent line scanner (`internal/vbnet`, #6362) plus an extractor (`internal/extractors/vbnet`, #6381/#6383). There is no usable tree-sitter VB.NET grammar: exactly one community grammar exists, it parses 41 of 86 constructs, and `Inherits`, `Implements`, `Handles` and generics are all among its failures — so it recovers ~90% of entities and 0% of the inheritance edges. A populated-but-wrong graph is worse than an empty one, which is why the scanner was written instead.
- **Entities and edges.** Class, Module, Structure, Interface, Delegate, Enum, Sub/Function/Property, fields and events, mapped onto the same SCOPE kinds the C# extractor uses so a mixed .NET repo indexes uniformly. Edges: `CONTAINS`, `IMPORTS`, `EXTENDS`, `IMPLEMENTS`, `CALLS`, `REFERENCES` — all emitted embedded on the owning record, so hierarchy edges anchor to the type rather than the file (the #6295/#6298 defect class). VB reuses `(` for invocation, array indexing and generic type arguments, so call-vs-index is decided by the per-file declaration table, never by a `name\s*\(` pattern; a pattern would emit a phantom `CALLS` edge on every array access. MEASURED: `CONTAINS` 13,205/13,205 resolved, `CALLS` 8,044 total / 4,166 resolved (52%, and see the `#6454` note below for why that rate is LOWER than the 5,059/3,386 = 67% this release originally shipped), `EXTENDS` 153/289, type-level `IMPLEMENTS` 72 emitted / 15 resolved, `IMPORTS` 763 emitted / 1 resolved.
- **A graded golden fixture and a corpus quality gate (#6406)** — `vbnet-mini`, with an `expected.json` that fails in both directions, plus corpus tests that publish the real-tree numbers rather than asserting a shape.
- **Partial classes split across `.vb` and `.Designer.vb` merge into one entity (#6412).** 88 of the 302 corpus files are `*.Designer.vb`, and a class split across the pair previously surfaced as two Components. MEASURED: 1,046 distinct `SCOPE.Component` ids before, 1,012 after — 34 duplicates removed. The residual 34 are dominated by same-named classes in *different* projects that must stay distinct (`MySettings` ×5, `Resources` ×5 in WakeOnLAN, one per `.vbproj`), plus 3 genuinely split types that need a compilation-unit (`.vbproj`) model grafel does not have. `Foo.aspx.designer.vb` pairs with `Foo.aspx.vb` through the same path.
- **`Handles` and `AddressOf` become `REFERENCES` edges (#6426):** 0 before, **1,212** after, 78.5% resolved. Both constructs name a member as a value rather than invoking it, which is what `REFERENCES` already means in the Go, Java and Python extractors; `CALLS` was rejected because no control transfer happens at either site. The construct is stamped as `via` = `handles` | `addressof`.
- **Method-level `Implements IFoo.Bar` becomes an `IMPLEMENTS` edge (#6445):** 0 before, **137 emitted / 32 resolved (23%)** after; type-level is unchanged at 72/15. Member-level clauses are **2.1×** more common than type-level in the corpus (137 vs 65), and the member edge is the only thing in the graph that says *which* member satisfies *which* interface member — a question the type-level edge cannot express and one VB.NET answers explicitly. Stamped `via=implements-member`. The 105 residual are dominated by framework interfaces (`IDisposable.Dispose` 13, `IValueConverter.Convert`/`ConvertBack` 13 each).
- **Member calls bind through the receiver's DECLARED TYPE (#6454):** `CALLS` **5,059 -> 8,044** total, **3,386 -> 4,166** resolved. `writer.WriteStartElement()` rendered its target from the receiver's *spelling*, so it could never join `XmlWriter.WriteStartElement`; it now renders from the declared type already recorded in the per-file symbol table, with no cross-file inference. The ceiling was larger than the issue described: `classify` answered `ParenUnknown` for every non-`Me` qualifier, and a `ParenUnknown` off statement head is not a call, so a qualified member call previously emitted **no `CALLS` edge at all** rather than a mis-named one. READ THE RATE HONESTLY: `CALLS` resolution FALLS 67% -> 52% and the disposition **bug rate rises 0.0918 -> 0.1666** (bug-extractor 1,830 -> 3,919, bug-resolver 143 -> 160, external-known 1,020 -> 1,115), because 2,985 real call sites that were previously refused now appear and most name a .NET Framework type that dangles until #6337. Strictly additive: an independent edge-set diff found 2,994 added, **0 removed, 0 retargeted**. Precision of the new sites, measured rather than asserted: on a uniform random sample of 120, **~5% (95% CI 1.9-10.6%)** are array indexes or default-property accesses rather than invocations; on the resolved subset **96.5%** land on a genuine method and 3.5% are the wrong edge kind. Scope is explicit `As` declarations only — `Dim x = expr` inference, method-return chaining and `With`-block receivers are still refused, as are receivers whose visible declarations disagree on the type or sit outside a nested type's reach.

**Known limits, with the measurements behind them.** These are the reasons a VB.NET graph will look thinner than a Go or Java one; each is a recorded decision, not an unknown.

- **Inheritance from .NET Framework base types does not resolve.** In the reporter's codebase 36 of 37 `Inherits` clauses point at a framework base (`System.Web.UI.Page`, `System.Windows.Forms.Form`); on the 302-file corpus `EXTENDS` resolves 153 of 289 for the same reason. Inheritance *within* the indexed tree resolves normally — the unresolved half is entirely bases with no source in the tree, and it is classified as external-known rather than as an extractor bug. Blocked on #6337 (external synthesis for `EXTENDS`/`IMPLEMENTS`), which is not VB-specific.
- **`IMPORTS` almost never resolves, and that is correct.** 1 of 763 edges binds in-tree, because VB.NET files import .NET Framework namespaces (`System.Windows.Forms`, `Microsoft.Win32`) that have no source in the indexed tree. Project-internal namespace imports do resolve.
- **`With`-block member accesses emit no edge.** 50 `With` blocks, 281 member sites, 112 of them calls. They are dropped cleanly, never bound to a wrong receiver. Building it was simulated end to end before being closed unbuilt: 112 call sites collapse to 25 `CALLS` edges under real dedup, of which **1** resolves — +0.5% edges, +0.03% resolved, and 24 new dangling edges. The recall behind them was #6454, which is now FIXED (see above) — and the two decisions should be compared in the same currency: S7d bought 1 resolved edge for 24 dangling ones, #6454 bought 780 for 2,205. That ratio, not the sign of the disposition delta, is what separated them.
- **Two overloads of the same method in one type share an entity id (#6440),** because the rendered declaration name omits the parameter list, so they merge into one node and each other's edges. 4 collisions in the corpus.
- **ASP.NET WebForms markup (`.aspx`, `.ascx`) is not parsed** — but `.aspx.vb` code-behind **is** fully extracted, and `.aspx.designer.vb` pairs with it through the partial-class merge. The double extension never broke classification. What is missing is markup-attribute handler wiring (`OnClick="btnSave_Click"`), which is the C#-idiomatic form; the VB-idiomatic `Handles btnSave.Click` is emitted. Closed unbuilt with the reversal trigger recorded on #6327.

### Fixed

#### Frameworks and cross-stack links

- **Spring MVC endpoints now point at the handler method instead of at themselves (#6429)** — thanks @auxmedrano. `spring_routes.go` had the handler method name in scope and discarded it at emit time, stamping a `Controller:<method>` stub that no entity kind matched; the endpoint synthesizer then stamped `source_handler = "Route:<path>"`, so the endpoint referenced its own route path. Every Java Spring `ROUTES_TO` hop dangled and endpoint→handler traces dead-ended one hop in. Routes now carry `handler_method`/`handler_class` and target `SCOPE.Operation:<Class>.<method>`, the identity the Java symbol extractor actually lands. Kotlin was already correct — this was a Java/Kotlin asymmetry, not a Spring-wide gap.
- **Angular `HttpClient` call sites are extracted, so the frontend half of a cross-stack link exists (#6433)** — thanks @auxmedrano. The reporter measured 98 `SCOPE.ExternalAPI` entities across a monorepo, all backend and zero frontend, against 42 frontend files making concrete `this.http.get/post/patch(...)` calls. Four independent guards each rejected idiomatic Angular: the JS/TS client patterns anchored on the bare identifiers `fetch`/`axios`, the Nest synthesizer required the literal token `httpService`/`HttpService` in the file text, and the REST early-exit guard rejected any file containing no `fetch(`, no `axios.` and no `$`. All four now admit receiver-style HTTP clients.
- **FastAPI routers mounted with `include_router(prefix=…)` no longer emit short paths (#6385)** — thanks @arthurgeron. Only the `APIRouter(prefix=…)` constructor form was read; nothing looked at the mount site's `prefix=` kwarg. Each mount site now emits an additive `http_endpoint` synthetic carrying `pattern_type: url_mount_point` and `url_prefix`, which the linker's existing prefix retry already consumes. This arm is strictly per-file; folding the prefix into the route path when the constructor and the mount live in different files is tracked as #6414.
- Together the Spring and Angular fixes close **#6313** (`http_endpoint_definition` orphaned at 99.6% despite recognized HTTP frameworks) — thanks @manuel1358000.

#### Language extraction

- **`.proto` files produce entities at all (#6356).** Every `.proto` file grafel had ever indexed produced zero entities: a complete, tested protobuf extractor registered itself under the token `proto` while the classifier routes the extension to `protobuf`, so lookup found nothing and every file was discovered, classified, bucketed, handed to a subprocess and dropped as skipped. The tree-sitter language table had the same split. Both halves are fixed together, and the dangling targets the extractor would have injected were fixed before it was turned on.
- **protobuf `oneof` fields are no longer silently dropped, and rpc type edges bind (#6358, #6359).** The message walker looked only at the direct children of `message_body`, and a `oneof`'s members nest one level deeper — so every field inside every `oneof` was invisible: no entity, no `CONTAINS`, no `REFERENCES`, no warning, on files parsing at a 0.0000 error ratio. Separately, rpc request/response edges were bare-name `IMPORTS` anchored on the file, duplicated, and could be the literal string `?`.
- **protobuf: a message sharing a name with an rpc keeps its file `CONTAINS` (#6422).** The file-contains relationship addressed every top-level declaration through the operation form, which is right for a service and wrong for a message or enum; because entity ids hash `(repo, kind, name, sourceFile)` and not subtype, a `message User` alongside `rpc User` had its `CONTAINS` silently reparented onto the rpc.
- **Solidity: a contract with base-constructor arguments no longer loses its whole file (#6423).** `contract X is ERC20("n","s")` matched nothing, because the inheritance capture group could not contain `(` and the pattern then required `{` — and since only matched bodies are walked, the contract and every member inside it vanished. On the graded `solidity-mini` fixture this moves entity recall from **19/40 to 40/40** and relationships from **8/13 to 13/13**, with forbidden hits still 0.
- **Solidity import placeholders no longer shadow real declarations (#6368).** One `SCOPE.Component` was minted per import path, deduped by path but named by basename; since entity ids exclude subtype and line span, a placeholder and the declaration it shadowed could share one id and whichever write landed last decided the node's span — so the graph could hand back an `import` line for a real contract. The `IMPORTS` edge now hangs on the per-file component carrier.
- **One import can no longer unresolve every edge to that name repo-wide (#6369).** The resolver indexed the per-import placeholder record — minted by a dozen extractors (css/scss/less, graphql, vue, kotlin, razor, proto, markdown, cpp, just, fish, javascript, cross/imports) — into its by-name table exactly like a real declaration. Adding one file whose import's last segment collided with a base type name flipped that name ambiguous and dropped every bare-name `EXTENDS` to it across the repo, including edges in files that imported nothing. A real declaration now outranks a placeholder.
- **F# graphs have inheritance topology (#6326).** The `inherit` and `interface` tokens were recognized and then dropped, and F# was absent from the cross-language hierarchy pass, so the extractor emitted no `EXTENDS` or `IMPLEMENTS` at all — "what extends this type" returned empty for every F# type, indistinguishable from "nothing does".
- **F# call-site line numbers below a block comment are correct (#6336).** The line stamp was counted over the comment-scrubbed copy of the file, so every `CALLS` edge below a block comment carried a line number that was too small.
- **Groovy graphs have inheritance topology (#6370).** Groovy was in neither of the two paths by which a language gets `EXTENDS`/`IMPLEMENTS`. Measured on the checked-in fixtures alone, 3 `EXTENDS` and 2 `IMPLEMENTS` were being dropped; the edges are now emitted from the Groovy extractor itself, so no duplicate component per type is minted.
- **Svelte relationships no longer dangle (#6366).** The Svelte extractor emits no file entity, so eleven sites across `RENDERS`, `USES`, `NAVIGATES_TO` and `CONTAINS` passed a raw path as the edge source and it reached the graph verbatim. Measured before the fix: **15 of 15 edges dangled** on the from side, across all four kinds.
- **Verilog `EXTENDS` anchors to the class, and Astro edges resolve (#6298).** Verilog passed the file path as the edge source, which was rewritten onto the file entity — measured on two classes in one `.sv` file, both `EXTENDS` edges came out on a single id, the file's, with their base lists merged onto that one node. Astro dangled its edges outright. Same shape @arthurgeron fixed for Solidity in #6297.
- **HCL `CONTAINS`/`DEPENDS_ON` resolve at nested paths (#6367).** Measured on one fixture at two paths: at `infra/envs/prod/main.tf` all 5 `CONTAINS` and both `DEPENDS_ON` dangled; at the repo root the same edges resolved onto the file component by accident, because the file component is named by basename and a path-valued source matches only when the path is its own basename.
- **Razor and Vue imports hand the resolver a full module path, not a basename (#6372).** The import resolver read the entity name and a `module` property that nothing populates on either indexing path, and never read the qualified name — so an extractor carrying the full path there had its relative-import rewrite declined and its module recorded wrong, both silently.
- **Files with a syntax error no longer emit entities from the broken region (#6360).** Extraction now skips `ERROR` subtrees, so what is emitted is a true subset of what the file declares. The silent-incompleteness half — a file that parses partially still reports as healthy — remains open on #6360.
- **12 more extensions are reported as unsupported rather than dropped in silence (#6344)**, including Oracle PL/SQL package bodies (`.pks`, `.pkb`, `.plb`), Apex triggers, XSLT and Arduino sketches. `.sql` is routed and extracted, so an Oracle shop got a populated-looking graph while the files holding the procedural logic were skipped without a word — silently partial coverage reads as complete coverage.
- **Generated-code banners behind a leading statement are detected (#6348).** The marker scan stopped at the first non-comment line, so a banner sitting below `set -e`, a `using`/`Imports` block or a module docstring was invisible.

#### Search and navigation

- **`grafel_related direction=uses` traverses every relationship it names (#6314)** — thanks @manuel1358000. The route was pinned to a single edge kind, `NAVIGATES_TO`, so every `CALLS`, `USES`, `USES_HOOK` and `REFERENCES` edge in the model was unreachable from a parameter named `uses` — and the response said nothing about it, returning a confidently empty answer. All five kinds are now traversed in both directions.

#### Indexing, daemon, and platform

- **A repo refused for file-descriptor budget can recover (#6267).** The WARM→COLD transition never released its watch descriptors, and nothing else returns descriptors at scale, so a repo refused for want of budget retried once per ~65-minute idle cycle and failed every time for the life of the daemon — indexed and answering queries, but receiving zero filesystem events.
- **A directory listing that fails mid-walk is reconciled instead of abandoned (#6304).** The watcher silently gave up on the remainder of a listing, so grafel neither charged nor watched those paths — permanently.
- **`GRAFEL_HOME` isolation is all-or-nothing (#6331).** A partially-set isolation environment silently targeted the real `~/.grafel` installation, so a sandboxed index could write to the user's live one. grafel now refuses on a partial set and warns when isolation is requested against a real `HOME`.
- **31 `skip_patterns.yaml` files and their loader had never executed (#6330).** Six did not parse and 63% of the patterns could not match anything; enabling them as written would have dropped every Go test file. The config is now validated and loaded.
- **Rule-load failures are surfaced instead of discarded (#6341).** The engine's rule loader swallowed parse errors, so a malformed rule file silently removed its own detections — one had not parsed in 79 days without a single report.
- **The "over memory budget" banner compares like with like (#6324, #6322, #6323).** It measured live whole-process RSS against a delta-accounted, next-start admission budget, so it fired on a correctly configured daemon. The dashboard Settings screen also answered the RSS budget from its own 512 MB default rather than from configuration — so the two screens disagreed and saving the form quartered the budget — and the Operations screen re-resolved the budget on every 5-second poll (forking `sysctl`) while reporting a next-start value as if it were live.
- **`grafel status` reports serve/engine version skew (#6339).** The skew check was `doctor`-only and invisible in `status`; `doctor` could also echo an HTML doctype as the running version, making a broken probe indistinguishable from a real skew.
- **Windows: the daemon's self-update no longer pops a console window (#6325, #6320)** — #6320 thanks @marcus555. A third console source was missed, and the `.vbs` wrapper used to suppress it was ANSI-decoded, truncated in place and never validated — so a non-ASCII username silently killed the daemon.
- **Dashboard: the mode switcher trigger works again (#6318)** and **the configured RSS budget is reported (#6319)** — thanks @marcus555.

#### Reporting and documentation

- **The feedback issue template asks for the report as an attachment (#6376).** It prescribed pasting a ~12,000-character markdown report into a textarea; two reports (#6313, #6314) arrived with every second table row clipped at exactly 25 characters, and the clipped half was the half carrying the findings. Attaching the file is now the primary instruction, with paste kept as a stated fallback.
- **`gen` prunes stale by-language documentation pages (#6354).** Generation was write-only, so a page for a slug that stopped being derived survived a clean run forever, and the CI check (`git diff --exit-code`) cannot see an untracked file — so a newly emitted orphan was caught only if a summary happened to link it. Two orphan pages had shipped on `main`.
